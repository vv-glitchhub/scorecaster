...
function missingTable(error) {
  return error?.code === "42P01" || /does not exist|schema cache/i.test(String(error?.message || error || ""));
}
...
function applyAutonomyLevel(policy, controls) {
  const result = { ...policy };
  if (controls.allow_automatic_risk_tightening === false && result.state !== "PAUSED") {
    result.riskScale = 1;
    result.maxStakePercent = Math.max(result.maxStakePercent, 0.1);
  }
...
async function storeLearning(admin, userId, learning, policy, controls, now) {
  ...
  if (trimError && !/function.*does not exist/i.test(trimError.message || "")) throw trimError;
}
...
async function processUser(admin, origin, userId, systemHealth, globalBudget, now) {
  ...
  try {
    ...
    await Promise.all([
      storeLearning(admin, userId, learning, policy, context.controls, now),
