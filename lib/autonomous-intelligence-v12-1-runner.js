...
import { buildAutonomyJournal, buildAutonomyState } from "./autonomous-scorecaster-v12.mjs";
import { runAutonomousScorecasterV12 } from "./autonomous-scorecaster-v12-runner.js";
import { buildV121Control, buildV121Incidents } from "./autonomous-intelligence-v12-1.mjs";
...
async function migrationReady(admin) {
...
async function loadUserContext(admin, userId) {
  const [settingsResult, bankrollResult, historyResult, openResult, stateResult] = await Promise.all([
    ...
    admin.from("bets")
      ...
      .eq("user_id", userId).neq("status", "open").order("created_at", { ascending: false }).limit(HISTORY_LIMIT),
    admin.from("bets")
      .select("id,stake,league,sport,match,status,raw_pick")
      .eq("user_id", userId).eq("status", "open").order("created_at", { ascending: true }).limit(OPEN_LIMIT),
...
async function syncIncidents(admin, userId, control, now) {
  ...
  const { data: active, error: activeError } = await admin.from("autonomous_agent_incidents")
    .select("id,fingerprint").eq("user_id", userId).eq("active", true);
  if (activeError) throw activeError;
  for (const row of active || []) {
    if (fingerprints.has(row.fingerprint)) continue;
    const { error } = await admin.from("autonomous_agent_incidents")
      .update({ active: false, resolved_at: now.toISOString(), last_seen_at: now.toISOString() }).eq("id", row.id);
    if (error) throw error;
  }
...
async function persistControl(admin, userId, context, baseState, control, runId, now) {
  const promotion = control.modelLab.promotion;
  ...
  if (runId) {
    const { data: run, error: readError } = await admin.from("autonomous_agent_runs").select("summary").eq("id", runId).maybeSingle();
    if (readError) throw readError;
    const journal = buildAutonomyJournal({ state: baseState });
    const { error: runError } = await admin.from("autonomous_agent_runs").update({
      ...
      summary: {
        ...(run?.summary || {}),
        agentVersion: "Autonomous-Intelligence-V12.1",
        autonomyV12: baseState,
        autonomyV121: control,
  ...
  return { snapshotId: snapshot.id, incidentCount, nextCheckAt };
}

async function insertFrozenRun(admin, userId, context, baseState, control, now) {
  const journal = buildAutonomyJournal({ state: baseState });
  const incidents = buildV121Incidents(control);
  const { data, error } = await admin.from("autonomous_agent_runs").insert({
    ...
    summary: {
      ...
      realMoneyBetting: false,
      agentVersion: "Autonomous-Intelligence-V12.1",
      failureStage: "v12.1_preflight",
      autonomyV12: baseState,
      autonomyV121: control,
      autonomyJournal: journal,
      probabilityChangedByAutonomy: false
    },
    error: clean(`V12.1 circuit breaker: ${control.blockers.join(", ") || control.killSwitchReason}`, 500),
...
async function enhancedPreflight(admin, origin, providerContext, now) {
  ...
  const { data, error } = await admin.from("autonomous_agent_state")
    ...
    .order("next_check_at", { ascending: true }).limit(PREFLIGHT_LIMIT);
  if (error) throw error;
  ...
  for (const row of data || []) {
    try {
      const context = await loadUserContext(admin, row.user_id);
      if (!context.settings?.enabled) continue;
      const decisions = await loadCurrentPicks(origin, context.settings, cache);
      const baseState = buildAutonomyState({
      ...
      const control = buildV121Control({
        baseState,
        history: context.history,
        bankroll: context.bankroll.bankroll,
        modelLab: context.modelLab,
      ...
      checked.push({ userId: row.user_id, context, baseState, control });
      if (control.mode === "FROZEN") {
        const runId = await insertFrozenRun(admin, row.user_id, context, baseState, control, now);
        const persisted = await persistControl(admin, row.user_id, context, baseState, control, runId, now);
        frozen.push({ userId: row.user_id, runId, ...persisted, reason: control.killSwitchReason });
      }
...
async function persistCompletedRuns(admin, preflight, startedAt, providerContext, now) {
  ...
  for (const run of runs || []) {
    if (run.summary?.autonomyV121 || run.summary?.failureStage === "v12.1_preflight") continue;
    try {
      let item = preflightByUser.get(run.user_id);
      if (!item) {
        const context = await loadUserContext(admin, run.user_id);
        const baseState = buildAutonomyState({ history: context.history, modelLab: context.modelLab, bankroll: context.bankroll, openBets: context.openBets, now });
        const control = buildV121Control({
          baseState,
          history: context.history,
          bankroll: context.bankroll.bankroll,
        ...
        item = { context, baseState, control };
      }
      await persistControl(admin, run.user_id, item.context, item.baseState, item.control, run.id, now);
      persisted += 1;
    } catch {
      failures += 1;
    }
...
export async function runAutonomousIntelligenceV121({ admin, origin, now = new Date() } = {}) {
  if (!admin) throw new Error("Autonomous Intelligence V12.1 requires a Supabase admin client");
  if (!origin) throw new Error("Autonomous Intelligence V12.1 requires a request origin");
  const startedAt = now instanceof Date ? now : new Date(now);
  if (Number.isNaN(startedAt.getTime())) throw new Error("Autonomous Intelligence V12.1 requires a valid clock value");

  const active = await migrationReady(admin);
  if (!active) {
    const base = await runAutonomousScorecasterV12({ admin, origin, now: startedAt });
    return {
      ...base,
      version: "autonomous-intelligence-v12.1-fallback-v12",
      v121MigrationActive: false,
      warning: "V12.1 persistence is not active; the reviewed V12 Daily Governor completed safely."
    };
  }

  const providerContext = await loadProviderContext(admin);
  const preflight = await enhancedPreflight(admin, origin, providerContext, startedAt);
  const base = await runAutonomousScorecasterV12({ admin, origin, now: startedAt });
  const persistence = await persistCompletedRuns(admin, preflight, startedAt, providerContext, new Date());

  return {
    ...base,
    version: "autonomous-intelligence-v12.1",
    v121MigrationActive: true,
    paperOnly: true,
    realMoneyBetting: false,
    v121: {
      enhancedPreflightUsers: preflight.checked.length,
      enhancedCircuitBreakerUsers: preflight.frozen.length,
      enhancedPreflightFailures: preflight.failed.length,
      persistedLearningSnapshots: persistence.persisted,
      ...
      safety: {
        existingDailyGovernorPreserved: true,
        persistentLearning: true,
        adaptiveScheduling: true,
        userConfiguredDrawdownAndLossLimits: true,
        providerHealthCircuitBreaker: true,
        persistentChampionChallenger: true,
