...
function maximumDrawdown(rows = []) {
  ...
  let peak = 0;
  let worst = 0;
  const averageStake = rows.reduce((sum, item) => sum + Math.max(0, finite(item.stake)), 0) / Math.max(1, rows.length);
  for (const row of rows) {
    equity += resultPnl(row);
    peak = Math.max(peak, equity);
    const denominator = Math.max(1, Math.abs(peak) + averageStake);
...
export function buildSystemGuard({ decisionAlerts = [], unifiedIncidents = [], diagnostics = null, settings = {} } = {}) {
  const config = normalizeAutonomousV2Settings(settings);
  const active = [...decisionAlerts, ...unifiedIncidents].filter((item) => item?.active !== false);
  const blockingTypes = new Set(["provider_outage", "provider_health", "all_skip", "stale_data", "weak_coverage"]);
  const blockers = active.filter((item) => item?.severity === "high" || blockingTypes.has(item?.alert_type || item?.alertType || item?.incident_type || item?.incidentType));
  const providerStatus = diagnostics?.provider_health?.status || diagnostics?.providerHealth?.status || null;
  const reasons = [];
  if (config.autoPauseOnIncident && blockers.length) reasons.push("active_system_incident");
  if (providerStatus === "offline" || providerStatus === "blocked") reasons.push("provider_health_blocked");
  return {
    version: "autonomous-system-guard-v2",
    ...
    blockingIncidents: blockers.slice(0, 10).map((item) => ({
      type: clean(item?.alert_type || item?.alertType || item?.incident_type || item?.incidentType, 100),
