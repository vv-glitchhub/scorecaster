...
function coverage(metrics, anchors) {
  const available = new Set(list(metrics));
...
function stage({ providerConfigured, acceptedModels, rejectedModels, metricCoverage, calibrationReadyModels }) {
  ...
  if (metricCoverage?.rate !== null && metricCoverage.rate < 0.67) return "provider-configured-metric-gap";
...
function familyReadiness(pick, providerConfiguration, sport, profile, family) {
...
export function buildAdvancedSignalReadinessV1(pick = {}, { providerConfiguration = {} } = {}) {
  ...
  const families = FAMILY_ORDER.map((family) => familyReadiness(pick, providerConfiguration, sport, profile, family));
