...
function providerRowsForInjuries(selectedLeague, rows = []) {
  if (!Array.isArray(rows)) return [];
  if (selectedLeague !== "wnba") return rows;
  return rows.filter((row) => {
    const status = String(row?.InjuryStatus || "").trim();
...
async function cachedSportsData(path, cacheKey, ttlMs) {
...
async function fetchTeamDirectory(selectedLeague) {
...
export async function fetchInjuriesForMatch({ homeTeam, awayTeam, sport, league }) {
  ...
  const [result, directory] = await Promise.all([
    ...
    fetchTeamDirectory(selectedLeague)
  ]);
  ...
  const rawItems = providerRowsForInjuries(selectedLeague, result.data);
  const filtered = [];
