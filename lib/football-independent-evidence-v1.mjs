...
export function buildFootballIndependentEvidenceV1(pick = {}, {
  ...
  const configuredSourceMatches = Boolean(entitlement.source) && sourceProviders.some((provider) => clean(provider, 100).toLowerCase() === entitlement.source.toLowerCase());
  const predictive = {
  ...
  const verified = predictiveSupport && supportingFamilies >= 1 && availability.injuryStatusLive && criticalConflicts.length === 0;
