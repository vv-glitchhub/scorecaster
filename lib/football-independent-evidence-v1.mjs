export const FOOTBALL_INDEPENDENT_EVIDENCE_VERSION = "football-independent-evidence-v1.1";

const HOUR_MS = 60 * 60 * 1000;
const PREDICTIVE_FRESHNESS_HOURS = 72;
const INJURY_REQUIRED_WITHIN_HOURS = 24;
const LINEUP_REQUIRED_WITHIN_HOURS = 6;
const MAX_SUPPORTIVE_GAP = 0.02;
const STRONG_CONFLICT_GAP = 0.05;
const MAX_MODEL_DISAGREEMENT = 0.12;
...
function buildOwnedPredictive(ownedDecision = {}, consensus = null) {
  const probability = finite(ownedDecision?.probability);
  const delta = probability === null || consensus === null ? null : probability - consensus;
  return {
    modelId: ownedDecision?.modelId || "scorecaster-own-football-baseline",
    status: ownedDecision?.qualified === true ? "ready" : ownedDecision?.applicable === true ? "not-qualified" : "unavailable",
    probability,
    marketConsensusProbability: consensus,
    probabilityDelta: delta === null ? null : Number(delta.toFixed(4)),
    supportsSelection: ownedDecision?.supportsSelection === true,
    strongConflict: ownedDecision?.strongConflict === true,
    inputSnapshotHash: ownedDecision?.predictionHash || null,
    chronologySafe: ownedDecision?.chronologySafe === true,
    noMarketInputs: ownedDecision?.independentFromMarket === true,
    preEventOnly: ownedDecision?.chronologySafe === true,
    providerCount: ownedDecision?.applicable === true ? 1 : 0,
    providers: ownedDecision?.applicable === true ? ["scorecaster-owned-baseline"] : [],
    marketSourceDetected: false,
    newestObservedAt: ownedDecision?.asOf || null,
    ageHours: finite(ownedDecision?.ageHours),
    fresh: ownedDecision?.fresh === true,
    entitlement: {
      configured: true,
      source: "scorecaster-owned-baseline",
      contract: "scorecaster-owned-model-v1",
      contractCompatible: true,
      commercialUseAllowed: true,
      modelUseAllowed: true,
      rawRedistributionAllowed: false,
      derivedAnalysisOnly: true
    },
    sourceMatchesConfiguredEntitlement: true,
    shotQuality: { available: false, complete: false, metricCount: 0, requestedMetricCount: 4 },
    confidence: finite(ownedDecision?.confidence),
    marketMapped: ownedDecision?.marketMapped === true,
    qualified: ownedDecision?.qualified === true,
    sourceType: "owned-model"
  };
}
...
export function buildFootballIndependentEvidenceV1(pick = {}, {
  ...
  const xgPredictive = {
    ...
    sourceType: "advanced-xg"
  };
  xgPredictive.qualified = Boolean(
    soccerModel?.status === "ready" &&
    modelProbability !== null &&
    advancedStatus?.ok === true &&
    xgPredictive.providerCount > 0 &&
    xgPredictive.inputSnapshotHash &&
    xgPredictive.chronologySafe &&
    xgPredictive.noMarketInputs &&
    xgPredictive.preEventOnly &&
    xgPredictive.fresh &&
    !xgPredictive.marketSourceDetected &&
  ...
  const ownedPredictive = buildOwnedPredictive(ownedDecision, consensus);
  const predictive = ownedPredictive.qualified ? ownedPredictive : xgPredictive;
  ...
  const injuryRequired = kickoffHours !== null && kickoffHours <= INJURY_REQUIRED_WITHIN_HOURS;
  ...
  if (xgPredictive.marketSourceDetected) criticalConflicts.push("A market-derived or otherwise blocked provider was detected in the xG predictive evidence lineage.");
  if (predictive.strongConflict) {
    const label = predictive.sourceType === "owned-model" ? "Scorecaster owned model" : "Independent xG model";
    criticalConflicts.push(`${label} is ${(Math.abs(Number(predictive.probabilityDelta || 0)) * 100).toFixed(1)} percentage points below the market consensus for the selected side.`);
  }
  if (ownedPredictive.qualified && xgPredictive.qualified && ownedPredictive.probability !== null && xgPredictive.probability !== null) {
    const disagreement = Math.abs(ownedPredictive.probability - xgPredictive.probability);
    if (disagreement > MAX_MODEL_DISAGREEMENT) {
      criticalConflicts.push(`Independent predictive models disagree by ${(disagreement * 100).toFixed(1)} percentage points.`);
    }
  }
  ...
  const injuryRequirementSatisfied = !availability.injuryRequired || availability.injuryStatusLive;
  const verified = predictiveSupport &&
    supportingFamilies >= 1 &&
    injuryRequirementSatisfied &&
    (!availability.lineupRequired || availability.lineups.bothConfirmed) &&
    criticalConflicts.length === 0;
  const anyEvidence = predictive.qualified || ownedPredictive.applicable === true || availability.verified || scheduleForm.verified || sportsReport?.readiness?.level === "partial" || sportsReport?.readiness?.level === "verified";
  ...
  const missing = [];
  if (!predictive.qualified) missing.push("a qualified chronology-safe independent predictive model");
  else if (!predictive.supportsSelection) missing.push("independent predictive support for the selected side");
  if (availability.injuryRequired && !availability.injuryStatusLive) missing.push("live verified injury/suspension status inside the final 24-hour window");
  ...
  return {
    ...
    policy: {
      ...
      ownedModelMayQualifyPredictiveFamily: true,
      supportingIndependentFamilyRequired: true,
      liveInjuryStatusRequiredWithinHours: INJURY_REQUIRED_WITHIN_HOURS,
      ...
      maximumIndependentModelDisagreement: MAX_MODEL_DISAGREEMENT,
