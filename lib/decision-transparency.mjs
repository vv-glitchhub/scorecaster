...
const clean = (value, limit = 240) => String(value ?? "").replace(/[\u0000-\u001f\u007f]/g, " ").replace(/\s+/g, " ").trim().slice(0, limit);
...
export const PUBLIC_FORMULAS = Object.freeze([
  ...
  },
  {
    ...
    note: "For example EV 0.05 means an estimated +5% expectation per unit over many comparable trials, not a promised result for one event."
  },
  {
    id: "freshness",
  ...
  },
  {
    ...
    note: "The ranking score orders available observations. It is not a probability and does not by itself authorize a paper pick."
  },
  {
    id: "kelly",
    name: "Kelly fraction",
    ...
    note: "Scorecaster caps full Kelly at 5% and primarily displays fractional Kelly in paper mode."
  },
  {
    id: "brier-score",
    name: "Brier score",
...
export function buildDecisionTransparency(records = [], pick = {}, now = Date.now(), env = process.env) {
  ...
  const odds = number(pick.bestOdds, number(oddsRow?.value));
  const marketProbability = number(pick.marketProbability, number(marketRow?.value, odds && odds > 1 ? 1 / odds : null));
  const modelProbability = number(pick.modelProbability, number(modelRow?.value));
  const decisionProbability = modelProbability ?? marketProbability;
  const edge = number(pick.edge, modelProbability !== null && marketProbability !== null ? modelProbability - marketProbability : null);
  ...
  const sources = uniqueSourceIds.map((id) => {
    ...
    return {
      ...
      newestObservationAt: [...sourceRows].sort((a, b) => new Date(observedAt(b) || 0) - new Date(observedAt(a) || 0))[0] ? observedAt([...sourceRows].sort((a, b) => new Date(observedAt(b) || 0) - new Date(observedAt(a) || 0))[0]) : null
    };
  });

  const verdict = clean(pick.decision || (quality < 0.55 ? "SKIP" : edge !== null && edge >= 0.04 && quality >= 0.72 ? "WATCH" : "CAUTION"), 20);
