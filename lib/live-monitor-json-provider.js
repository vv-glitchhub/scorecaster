...
export function liveMonitorProviderConfiguration(env = process.env) {
  ...
  return {
    ...
    productionAllowed: collection.allowed && publication.allowed,
    blockedReason: !collection.allowed ? collection.reason : !publication.allowed ? publication.reason : null,
    baseUrl: source?.baseUrl ? "configured" : null,
    hasApiKey: Boolean(env.LIVE_MONITOR_API_KEY || env.COLLECTOR_JSON_API_KEY),
