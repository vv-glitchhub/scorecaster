...
const round = (value, digits = 6) => value !== null && value !== undefined && value !== "" && Number.isFinite(Number(value)) ? Number(Number(value).toFixed(digits)) : null;
const finite = (value) => {
...
function analyzeSelection(rows, generatedMs, thresholds) {
  ...
  const causeLabel = current.probability === null || opening.probability === null ? "insufficient-current-evidence" : broad.detected
  ...
  return {
    ...
    movement: {
      probabilityChange: round(probabilityChange),
      priceChange: round(priceChange),
      direction: probabilityChange === null ? "unknown" : Math.abs(probabilityChange) < 0.0025 ? "stable" : probabilityChange > 0 ? "shortening" : "lengthening",
      causeLabel,
      broadEvidence: broad,
      staleProviders,
      outlierProviders,
