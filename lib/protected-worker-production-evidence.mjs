  ...
  return (Array.isArray(manifest.internalWorkers) ? manifest.internalWorkers : [])
    .map((worker) => ({
      ...
      allowedStatuses: Array.isArray(worker?.allowedStatuses)
        ? worker.allowedStatuses.map(integer).filter((value) => value !== null)
        : []
    }))
...
export function buildTrustedProtectedWorkerProbeEvidence({
  ...
  if (integer(implementation?.workerCount) !== expected.length) failures.push("worker-implementation-count-mismatch");
  ...
  return {
    ...
    passedWorkerCount: passed ? expected.length : 0,
