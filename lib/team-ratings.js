...
export const DEFAULT_TEAM_DATA = TEAM_RATINGS;
...
export function getTeamProfile(teamName) {
  return getTeamRating(teamName);
}

export function getOverallRating(teamName) {
  return getTeamRating(teamName).rating;
}

export function getAllTeamRatings() {
  return Object.entries(TEAM_RATINGS).map(([team, data]) => ({
    team,
    ...data,
  }));
}
...
export function getMatchRatingContext(match) {
  ...
  return {
    ...
    attackDefenseDiff:
      (home.attack - away.defense) - (away.attack - home.defense),
  };
}
