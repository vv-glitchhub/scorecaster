...
export function buildDecisionEvidenceContractV1(pick = {}) {
  ...
  const sportsAvailable = objectAvailable(pick.sportsIntelligence);
  ...
  const conflictCount = sportsAvailable ? conflicts.length : null;
  ...
  const payload = {
    ...
    known: {
      ...
      independentEvidence: {
        readiness: sportsAvailable ? text(readiness.level, 30) || "market-only" : "unavailable",
        verifiedCount: sportsAvailable ? number(readiness.verifiedCount) : null,
        totalChecks: sportsAvailable ? number(readiness.totalChecks) : null,
        sourceCount: sportsAvailable ? number(pick.sportsIntelligence?.sourceCount) : null,
        conflicts: conflictCount,
        relativeImpact: sportsAvailable ? number(pick.intelligenceRelativeImpact) : null
      }
    },
