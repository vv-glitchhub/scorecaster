...
import { auditModelLineageV1, MODEL_LINEAGE_GUARD_VERSION } from "./model-lineage-guard-v1.mjs";
...
function modelIdentity(candidate = {}, dependenceGroupOverride = null) {
...
function canonicalExternalCandidate(candidate = {}, pick = {}, horizon, now, origin = "external") {
  const audit = candidate.audit && typeof candidate.audit === "object" ? candidate.audit : {};
  const lineage = auditModelLineageV1(candidate, {
    sportKey: pick.sportKey || pick.league,
    requireLineage: true
  });
  const identity = modelIdentity(candidate, lineage.dependenceGroup);
  ...
  const warnings = [...performance.warnings, ...lineage.warnings];
...
function explicitCandidate(pick = {}) {
  ...
  return {
    ...
    signalFamilies: audit.signalFamilies || audit.signal_families,
    dataLineage: audit.dataLineage || audit.data_lineage,
    probability: p,
...
function formRestCandidate(pick = {}) {
  ...
  return {
    ...
    signalFamilies: ["historical-results", "workload"],
    dataLineage: {
      providers: [snapshot?.provider?.source || "thesportsdb"],
      metrics: ["recent-results", "score-margin", "rest", "schedule-congestion"]
    },
    probability: p,
...
function historicalRatingCandidate(pick = {}) {
  ...
  return {
    ...
    dependenceGroup: historicalDependenceGroup(sportKey),
    signalFamilies: ["historical-results", "rating"],
    dataLineage: {
      providers: [snapshot?.provider?.source || "thesportsdb"],
      metrics: ["completed-results", "elo-rating"]
    },
    probability: p,
    generatedAt: snapshot.generatedAt,
...
export function buildModelFactoryV1(pick = {}, { now = Date.now() } = {}) {
  ...
  return {
    ...
    rejectedModels: rejected.map((item) => ({
      ...
      claimedDependenceGroup: item.lineage?.claimedDependenceGroup || null,
      signalFamilies: item.lineage?.signalFamilies || [],
      lineageFingerprint: item.lineage?.lineageFingerprint || null,
