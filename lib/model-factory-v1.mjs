...
function canonicalPerformance(pick, candidate, identity, horizon, now) {
  ...
  const warnings = [...evidence.errors];
  return {
...
function formRestCandidate(pick = {}) {
  ...
  return {
    ...
    dependenceGroup: `${sportKey || "unknown"}-form-rest-logit-family`,
    probability: p,
...
function featureOnlyInventory(pick = {}) {
  ...
  return {
    ...
    dependenceGroup: "form-rest-features",
    probabilityOutputOwnedByFactory: false,
...
export function buildModelFactoryV1(pick = {}, { now = Date.now() } = {}) {
  ...
  for (const candidate of rawExternalCandidates(pick)) {
    const result = canonicalExternalCandidate(candidate, pick, horizon, now, "external-audited-output");
  ...
  const formRest = formRestCandidate(pick);
  if (formRest) {
    const result = canonicalExternalCandidate(formRest, pick, horizon, now, "form-rest-shadow-adapter");
    if (result.accepted) accepted.push(result.output);
    else rejected.push(result);
    warnings.push(...(result.performanceWarnings || []).map((warning) => `${result.identity?.modelId || "unknown"}:${warning}`));
  }
  ...
  const transparent = transparentInventory(pick);
  const featureOnly = featureOnlyInventory(pick);
  const inventory = [transparent, featureOnly].filter(Boolean);
