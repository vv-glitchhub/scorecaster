...
async function loadUserContext(admin, userId) {
  const [settingsResult, bankrollResult, openResult, historyResult] = await Promise.all([
  ...
  const error = settingsResult.error || bankrollResult.error || openResult.error || historyResult.error;
  if (error) throw error;
  return {
...
async function finishRun(admin, runId, result, summary) {
...
function chooseDecisions(decisions, context) {
  ...
  const usedEvents = new Set(exposure.events);
  const selected = [];
  const skipped = [];
  ...
  for (const decision of decisions) {
    ...
    if (finite(decision.priorityScore) < settings.minPriorityScore) reasons.push("priority_below_user_threshold");
    ...
    if (event && usedEvents.has(event)) reasons.push("event_already_exposed");
    if (selected.length >= settings.dailyPickLimit) reasons.push("daily_pick_limit");
    if (reasons.length) {
    ...
    if (stake < 0.01) {
      skipped.push({ decision, reasons: [remainingTotal < 0.01 ? "total_exposure_full" : "league_exposure_full"] });
      continue;
    }
...
function clientRef(userId, decision, now) {
  ...
  return `autonomous-v1-${day}-${digest}`;
}

function paperRow(userId, runId, decision, source, fixtureSource, now) {
  const odds = finite(decision.odds);
  ...
  return {
    ...
    status: "open",
    raw_pick: {
      source: "scorecaster-autonomous-v1",
      eventId,
      ...
      agentVersion: "Autonomous-Paper-Agent-V1",
...
function summaryDecision(decision, saved, reason = null) {
  return {
...
async function processUser(admin, entry, source, now, globalBudget) {
  ...
  const choice = chooseDecisions(governed, entry.context);
  ...
  for (const decision of selected) {
    const result = await saveDecision(admin, paperRow(entry.userId, runId, decision, source.source, source.fixtureSource, now));
    if (result.saved) {
      savedCount += 1;
      totalStake += decision.autonomousStake;
      audit.push(summaryDecision(decision, true));
  ...
  await finishRun(admin, runId, result, summary);
  await completeUser(admin, entry.userId, result);
  return result;
}

export async function runAutonomousPaperAgent({ admin, origin, now = new Date() } = {}) {
  if (!admin) throw new Error("Autonomous Paper Agent requires a Supabase admin client");
  if (!origin) throw new Error("Autonomous Paper Agent requires a request origin");
  const startedAt = now instanceof Date ? now : new Date(now);
  if (Number.isNaN(startedAt.getTime())) throw new Error("Autonomous Paper Agent requires a valid clock value");
  ...
  for (const userId of userIds) {
    try {
      ...
      await completeUser(admin, userId, { status: "error", error: text(error?.message, 500, "Autonomous Agent context could not be loaded") });
    }
  }
  ...
  for (const entry of selectedEntries) {
    if (!sourceCache.has(entry.sourceKey)) {
  ...
  for (const entry of selectedEntries) {
    try {
      ...
      await completeUser(admin, entry.userId, { status: "error", error: text(error?.message, 500, "Autonomous Agent processing failed") });
    }
  }

  return {
    ok: true,
    version: "autonomous-paper-agent-v1",
    paperOnly: true,
    realMoneyBetting: false,
...
export const AUTONOMOUS_AGENT_LIMITS = {
  usersPerRun: MAX_USERS_PER_RUN,
  sportsPerUser: MAX_SPORTS_PER_USER,
  sourceGroupsPerRun: MAX_SOURCE_GROUPS_PER_RUN,
  picksPerUser: MAX_PICKS_PER_USER,
