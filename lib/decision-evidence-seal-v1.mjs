...
function agentDecision(value) {
  const normalized = text(value, 20).toUpperCase();
  if (normalized === "CAUTION") return "WATCH";
  return ["PLAY", "WATCH", "SKIP"].includes(normalized) ? normalized : null;
}

function decisionCompatible(product, agent) {
  if (!agent) return true;
  if (agent === "PLAY") return product === "PLAY";
  if (agent === "WATCH") return product === "PLAY" || product === "CAUTION";
  return agent === "SKIP";
}
...
export function sanitizeDecisionEvidenceSealV1(value, context = {}) {
  ...
  const expectedDecision = agentDecision(context.decision);
  const expectedSelection = text(context.selection, 160) || null;
  const expectedEventId = text(context.eventId, 180) || null;
  if (!decisionCompatible(payload.productDecision, expectedDecision)) return null;
...
export function decisionEvidenceBoundaryText(value) {
  ...
  if (!seal) return "";
  return [
    `Evidence ${seal.contractFingerprint}; seal ${seal.sealFingerprint}.`,
    "Research non-voting.",
    "Context cannot upgrade.",
    "Paper-only."
  ].join(" ");
}
