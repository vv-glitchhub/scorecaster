...
} from "./verified-live-monitor-core-v1.mjs";
...
const supportedSport = (sport) => SUPPORTED_SPORT_PREFIXES.some((prefix) => sport === prefix || sport.startsWith(`${prefix}_`));
...
function auditRow(row, eventId, generatedAt, index) {
  ...
  for (const timestamp of [observedAt, providerUpdatedAt, capturedAt]) {
    if (timestamp && Date.parse(timestamp) > Date.parse(generatedAt) + 5000) errors.push("future-timestamp");
  }
