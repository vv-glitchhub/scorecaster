...
export function simulateSingleMatch({
  ...
  return {
    ...
    draw: draws / n,
    away: awayWins / n,
    homePct: (homeWins / n) * 100,
    drawPct: (draws / n) * 100,
    awayPct: (awayWins / n) * 100,
  };
}

/**
 * Backwards-compatible export.
 * Vanha MatchSimulatorPanel käyttää tätä nimeä.
 */
export function runMatchSimulation(match, iterations = 10000) {
  const safeMatch = match || {};

  const homeTeam = safeMatch.home_team || safeMatch.homeTeam || "Home";
  const awayTeam = safeMatch.away_team || safeMatch.awayTeam || "Away";

  const probabilities =
    safeMatch.probabilities ||
    probabilitiesFromOdds(safeMatch.bestOdds || safeMatch.odds || {});

  return simulateSingleMatch({
    homeTeam,
    awayTeam,
    probabilities,
    iterations,
