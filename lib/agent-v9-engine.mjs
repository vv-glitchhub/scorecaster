...
function buildCounterArguments(pick, stress, missing) {
  const argumentsList = [
    `Jos todellinen todennäköisyys on epävarmuusvälin alarajalla, EV on ${(stress.downsideEv * 100).toFixed(1)} %.`,
    `Jos tarjottu kerroin laskee alle ${stress.targetPlayOdds.toFixed(2)}, 3 % tavoite-EV ei enää täyty.`,
...
function calculateConservativeStake(pick, decision, bankroll, maxStakePercent, stress) {
...
export function buildAgentV9Decision({
  ...
  if (confidence < 0.55) blockers.push("dataconfidence on alle 55 %");
  if (trust < 0.62) blockers.push("trust score on alle 62/100");
  if (edge < 0.02 || ev < 0.03) blockers.push("edge tai EV ei täytä PLAY-rajaa");
  if (!stress.robustPositive) blockers.push("stressatun todennäköisyysalarajan EV ei ole positiivinen");
  if (stress.halfWidth > 0.12) blockers.push("todennäköisyyden epävarmuusväli on liian leveä");
  ...
  const counterArguments = buildCounterArguments(pick, stress, missingEvidence);
  ...
  return {
    ...
    priorityScore,
    robustnessScore,
    suggestedStake: calculateConservativeStake(pick, decision, safeBankroll, safeMaxStakePercent, stress),
    ...
    decisionReason: decision === "PLAY"
      ? "Kohde läpäisee myös epävarmuusalarajan, datan laatuportin ja konservatiivisen panosrajan."
      : decision === "WATCH"
        ? `Kohde ei läpäise kaikkia Agent V9:n vastatestejä: ${blockers.join(", ") || "lisävahvistus tarvitaan"}.`
    ...
    priceGuard: {
      ...
      minimumPlayOdds: stress.targetPlayOdds,
      conservativeBreakEvenOdds: stress.conservativeBreakEvenOdds,
      buffer: stress.priceBuffer
    },
...
export function buildAgentV9Portfolio(picks = [], {
  ...
  const decisions = (Array.isArray(picks) ? picks : [])
    .map((pick) => buildAgentV9Decision({ pick, learning, bankroll: safeBankroll, maxStakePercent }))
