...
export function splitMatchesByDataStatus(matches = []) {
  return {
    bettable: matches.filter((m) => getMatchDataStatus(m).level === "bettable"),
    partial: matches.filter((m) => getMatchDataStatus(m).level === "partial"),
    fixturesOnly: matches.filter((m) => getMatchDataStatus(m).level === "fixture_only"),
  };
}
