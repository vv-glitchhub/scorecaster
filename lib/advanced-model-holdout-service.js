...
export async function loadAdvancedModelHoldoutReport({ days = 90, now = Date.now() } = {}) {
  ...
  try {
    ...
    settled.forEach((item, index) => {
      if (item.status === "fulfilled" && item.value?.ok) results.push(...(item.value.results || []).map((result) => ({
        ...result,
        holdoutLeagueKey: `${requestedGroups[index].sportKey}|${requestedGroups[index].league}`
      })));
    ...
    const value = {
      ...
      days: windowDays,
      since,
      snapshotRowsScanned: allSnapshots.length,
      snapshotLimitReached: allSnapshots.length >= MAX_SNAPSHOTS,
      maximumSnapshotRows: MAX_SNAPSHOTS,
      leagueLimitReached: groups.size > MAX_LEAGUES,
