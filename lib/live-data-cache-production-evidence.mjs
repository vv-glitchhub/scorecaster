...
export function buildTrustedLiveDataCacheGateEvidence({
  ...
  const evidenceRef = safeEvidenceRef(raw?.evidenceRef);
  if (!evidenceRef) failures.push("evidence-reference-invalid-or-secret-bearing");
  ...
  const latestProbeAt = probes.map((probe) => probe.observedAt).filter(Boolean).sort().at(-1) || null;
  if (observedAt && latestProbeAt && Date.parse(observedAt) < Date.parse(latestProbeAt)) failures.push("evidence-observed-at-before-latest-probe");
  ...
  const manualGateEvidence = gateId ? {
    [gateId]: {
      ...
      evidenceRef: passed ? evidenceRef : null
    }
  } : {};
