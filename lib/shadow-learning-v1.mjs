...
function timestamp(row = {}, index = 0) {
  const explicit = finite(row.timestamp);
  ...
  return (Array.isArray(rows) ? rows : [])
    .map((row, index) => {
      ...
      return {
        ...
        sportKey: String(row.sport || row.sport_key || row.sportKey || "unknown"),
        marketKey: String(row.market || row.market_key || row.marketKey || "unknown"),
