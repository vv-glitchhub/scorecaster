...
function builtinSources(env = process.env) {
  const sportmonksTokenPresent = Boolean(clean(env.SPORTMONKS_API_TOKEN, 20));
  const sportmonksEnabled = bool(env.SPORTMONKS_FOOTBALL_EVIDENCE_ENABLED);
  const sportmonksCommercial = bool(env.SPORTMONKS_COMMERCIAL_USE_ALLOWED);
  const sportmonksModelUse = bool(env.SPORTMONKS_MODEL_USE_ALLOWED);
  return [
    normalizeSource({
    ...
    normalizeSource({
      ...
      sports: ["soccer"],
      updateCadence: "Scorecaster mirror every 6 hours; rotating historical bootstrap",
      ...
      restrictedFields: DEFAULT_RESTRICTED_FIELDS,
      notes: "CC0 fixture/result history mirrored into Scorecaster-owned canonical storage. Stored normalized history remains available if the upstream repository later changes or disappears; provenance and license are retained."
    }),
    normalizeSource({
      id: "the_odds_api",
      name: "The Odds API",
    ...
    normalizeSource({
      id: "sportmonks_football",
      name: "Sportmonks Football evidence",
      type: "api",
      accessMode: "production",
      enabled: sportmonksTokenPresent && sportmonksEnabled && sportmonksCommercial && sportmonksModelUse,
      commercialUseAllowed: sportmonksCommercial,
      redistributionAllowed: false,
      modelTrainingAllowed: sportmonksModelUse,
      attributionRequired: false,
      attribution: "Football evidence data: Sportmonks",
      license: "commercial product use allowed when subscribed; raw feed resale/redistribution requires separate approval",
      termsUrl: "https://www.sportmonks.com/terms-of-service/",
      baseUrl: "https://api.sportmonks.com/v3/football",
      sports: ["soccer"],
      updateCadence: "provider and subscription dependent; Scorecaster pregame capture every 30 minutes",
      ...
      restrictedFields: [...DEFAULT_RESTRICTED_FIELDS, "api_token", "rawFixture", "rawXgFixture", "rawStatistics", "rawLineup", "rawSidelined"],
      notes: "Fail-closed Football Independent Evidence adapter. Enabled only when token, feature flag, commercial-use flag and model-use flag are all set. Scorecaster stores normalized/derived pregame observations and hashes lineage; raw feed redistribution remains disabled."
    }),
    normalizeSource({
