...
function builtinSources(env = process.env) {
  return [
    ...
    normalizeSource({
      ...
      name: "Veikkaus public data discovery",
      type: "api",
      accessMode: "disabled",
      enabled: false,
      ...
      restrictedFields: [...DEFAULT_RESTRICTED_FIELDS, "session", "ticket", "cashOut", "customerContext", "undocumentedEndpoint"],
      notes: "Discovery placeholder only. No endpoint, terms or commercial/display rights have been verified. Screenshots and visible pages do not establish API reuse rights. Collection and publishing must remain disabled until an operator records the authoritative source, terms and permitted use."
    }),
    normalizeSource({
