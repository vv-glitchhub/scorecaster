...
async function loadExecutedSamples(admin, userId) {
...
async function loadRejectedCandidateSamples(admin, userId) {
  const { data, error } = await admin
    .from("autonomous_agent_decision_audit")
    .select("id,sport,league,market,model_version,model_probability,odds,price_clv,result,outcome_value,settled_at,created_at")
    .eq("user_id", userId)
    .eq("settlement_status", "settled")
    .is("saved_bet_id", null)
    .in("result", ["win", "loss"])
  ...
  return (data || []).map((row) => ({
    ...
    sport: row.sport,
    league: row.league,
    market: row.market || "h2h",
    model_version: row.model_version || "Autonomous-Scorecaster-V13",
    original_probability: row.model_probability,
    odds_at_selection: row.odds,
    stake: 0,
    closing_odds: null,
    clv: row.price_clv,
    result: row.result,
    ...
    observation_type: "rejected-shadow-candidate"
  }));
}

async function loadSamples(admin, userId) {
  const [executed, candidates] = await Promise.all([
    loadExecutedSamples(admin, userId),
    loadRejectedCandidateSamples(admin, userId)
  ]);
  return [...executed, ...candidates]
    .sort((left, right) => Date.parse(left.settled_at || left.created_at || 0) - Date.parse(right.settled_at || right.created_at || 0))
...
async function saveCycle(admin, userId, report) {
  const { data, error } = await admin
    ...
    .insert({
      ...
      safety: {
        ...report.safety,
        rejectedCandidateCalibrationIncluded: true,
        counterfactualProfitCalculated: false
      },
      report: {
        ...report,
        observationPolicy: {
          executedPaperSamples: true,
          rejectedShadowCandidates: true,
          rejectedCandidateStake: 0,
          rejectedCandidateProfit: 0,
          counterfactualProfitCalculated: false,
...
export async function runShadowLearningWorker({ admin } = {}) {
  ...
  return {
    ...
    version: "shadow-learning-worker-v2-candidate-observations",
    mode: "shadow-only",
    paperOnly: true,
