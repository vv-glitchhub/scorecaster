...
export function sanitizeAgentExplanationInput(input = {}) {
  ...
  const contract = {
    ...
    selection: text(source.selection || source.label, 120, "Selection"),
    ...
    evidence: withFallback(list(source.evidence), "The decision is based on Scorecaster's deterministic market and risk engine."),
    counterArguments: withFallback(list(source.counterArguments), "Market consensus can be wrong or change before the event."),
    missingEvidence: withFallback(list(source.missingEvidence), "market-data freshness immediately before the event"),
