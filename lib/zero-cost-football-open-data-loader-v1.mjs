...
export const ZERO_COST_FOOTBALL_OPEN_DATA_LOADER_VERSION = "scorecaster-zero-cost-football-open-data-loader-v1";
...
async function fetchWithTimeout(url, { responseType = "text", timeoutMs = 30_000, headers = {} } = {}) {
  const response = await fetch(url, {
    cache: "no-store",
    headers: {
      "User-Agent": "Scorecaster-Zero-Cost-Model-Lab/1.0",
...
function sumMatchXg(events = [], homeName, awayName) {
  let homeXg = 0;
  let awayXg = 0;
  let shots = 0;
  for (const event of Array.isArray(events) ? events : []) {
    ...
    if (team === canonicalFootballTeam(homeName)) homeXg += xg;
    else if (team === canonicalFootballTeam(awayName)) awayXg += xg;
    else continue;
    shots += 1;
  }
  return { homeXg, awayXg, shots };
}
...
export async function loadZeroCostFootballLabDataset(options = {}) {
  ...
  const xgMatches = await mapWithConcurrency(statsBombMatches, Number(options.concurrency) || 8, async (match) => {
    ...
    const xg = sumMatchXg(events, match.homeTeam, match.awayTeam);
    return { ...match, ...xg };
  });
  ...
  eventHashes.sort();
  const manifest = {
    version: ZERO_COST_FOOTBALL_OPEN_DATA_LOADER_VERSION,
    retrievedAt,
    experiment: "EPL 2015/2016 StatsBomb xG vs Football-Data historical market benchmark",
    statsBomb: {
