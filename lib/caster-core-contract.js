...
export function getScorecasterCoreState() {
  return createCasterAppState({
    app: 'scorecaster',
    name: 'Scorecaster',
    status: CASTER_APP_STATUS.LOCAL_READY,
    syncStatus: CASTER_SYNC_STATUS.LOCAL_ONLY,
    localStorageKeys: ['scorecaster.quickUse.bets'],
    routes: ['/', '/quick-use', '/risk', '/betting', '/analytics', '/tracking'],
    features: ['manual picks', 'local bet slip', 'risk decision', 'bankroll rules', 'bet slip engine'],
    nextActions: ['connect Supabase tracking', 'connect live odds', 'add daily top picks', 'sync to Caster-hub']
  });
}
