...
import { sportsAnalyticsProviderConfiguration } from "./sports-analytics-provider.js";
...
import { attachFootballIndependentEvidenceV1, buildFootballIndependentEvidenceV1 } from "./football-independent-evidence-v1.mjs";
...
export async function enrichPickWithLiveIntelligence(pick, { allowLiveSecondaryPricing = false } = {}) {
  ...
  const footballEvidence = buildFootballIndependentEvidenceV1(withMlbPitchingOffense, {
    sportsReport: report,
    soccerModel: withMlbPitchingOffense.soccerXgPoissonShadowV1,
    ownedDecision: withMlbPitchingOffense.ownedDecisionEvidenceV1,
    advancedStatus: withMlbPitchingOffense.advancedShadowInputStatus,
    advancedObservations: advancedShadowInputs.observations,
    formRest: withMlbPitchingOffense.formRestShadow,
    providerConfiguration: sportsAnalyticsProviderConfiguration(),
    now
  });
  const withFootballEvidence = attachFootballIndependentEvidenceV1({ ...withMlbPitchingOffense, sportsIntelligence: report }, footballEvidence);
  const fusedSportsReport = withFootballEvidence.sportsIntelligence || report;
  const withSportsEvidence = applySportsIntelligenceGate(withFootballEvidence, fusedSportsReport);
  const withPolymarket = applyPolymarketSafety(withSportsEvidence, polymarket);

  try {
    const unified = await loadUnifiedSportsData(withPolymarket, fusedSportsReport, { now, allowLiveSecondaryPricing });
    const enriched = {
    ...
    const ledger = buildUnifiedSportsDataLedgerWithLineupProvenance({ pick: withPolymarket, sportsReport: fusedSportsReport, now });
    const enriched = {
