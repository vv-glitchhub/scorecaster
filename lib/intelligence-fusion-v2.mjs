...
function chronologyState(factor = {}, pick = {}, now = Date.now()) {
  // Market freshness already has a dedicated gate. The legacy odds ledger may
  // use commenceTime as a display-time fallback when a provider timestamp is
  // absent, so treating that field as an evidence timestamp would create a
  // false future-data rejection. Contextual evidence still gets strict checks.
  if (clean(factor.key, 80) === "odds-consensus") {
    return { valid: true, checked: false, futureSourceCount: 0, governedBy: "market-freshness-gate" };
  }
  ...
  if (!sourceTimes.length) return { valid: true, checked: false, futureSourceCount: 0, governedBy: "context-chronology" };
  const futureSourceCount = sourceTimes.filter((value) => value > horizon + 60_000).length;
  return { valid: futureSourceCount === 0, checked: true, futureSourceCount, governedBy: "context-chronology" };
}
...
function dataQualityGate({ eligible, ignored, adverse, coverage, trust, conflicts, missing }) {
  const reasons = [];
...
export function buildIntelligenceFusionV2(pick = {}, { now = Date.now() } = {}) {
  ...
  const gate = dataQualityGate({ eligible, ignored, adverse, coverage, trust, conflicts, missing });
  ...
  return {
    ...
    rules: {
      ...
      marketFreshnessUsesExistingGate: true,
