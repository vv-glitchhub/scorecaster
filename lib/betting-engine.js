...
function sportBoost(match, side) {
  const key = String(match?.sport_key || "").toLowerCase();

  if (key.includes("tennis")) {
    return side === "home" ? 0.012 : 0.008;
  }

  if (key.includes("mma") || key.includes("boxing")) {
    return side === "home" ? 0.01 : 0.01;
  }
  ...
  return filtered
    ...
    .map((item) => {
      ...
      const analyzed = {
        ...
        shouldBet,
        risk,
        modelVersion: "v3-outright",
        strength: shouldBet ? "Outright value" : "Ei valuea",
        reasons: [
          "Outright-markkina analysoitu",
          "Korkea varianssi, panos pidettävä pienenä",
          "Model V3 käyttää varovaista outright-arviota",
        ],
      };
...
function finalizeRows(rows, match, bankroll, modelSummary = null) {
  return rows
    ...
    .map((row) => {
      ...
      const analyzed = {
        ...
        marketProb,
        marketProbability: marketProb,
        modelProbability: row.modelProb,
        edge,
        ev,
        stake,
        kelly: stake / bankroll,
        ...
        modelSummary,
        reasons: row.reasons || [
          "Model V3 vertaa omaa todennäköisyyttä markkinan todennäköisyyteen",
          "Edge on positiivinen suhteessa kertoimeen",
          "Panos lasketaan varovaisella Kelly-mallilla",
        ],
        strength:
          edge >= 0.07
            ? "Vahva"
    ...
    rows = [
      {
        ...
        reasons: [
          "Over/Under arvioidaan hyökkäys- ja puolustussignaalista",
          "Model V3 huomioi ottelun maali-/pistemäärän signaalin",
        ],
      },
      {
        key: "under",
        ...
        reasons: [
          "Under/Over arvioidaan hyökkäys- ja puolustussignaalista",
          "Model V3 etsii markkinasta väärin hinnoiteltua totalia",
        ],
      },
    ];
    ...
    rows = [
      {
        ...
        reasons: [
          "Handicap arvioidaan rating-erosta",
          "Model V3 huomioi kotijoukkueen vahvuuden suhteessa linjaan",
        ],
      },
      {
        key: "spread-away",
        ...
        reasons: [
          "Handicap arvioidaan rating-erosta",
          "Model V3 huomioi vierasjoukkueen vahvuuden suhteessa linjaan",
        ],
      },
    ];
  } else {
    ...
    rows = [
      {
        ...
        modelProb: Math.min(0.97, model.home + sportBoost(match, "home")),
        reasons: [
          "Model V3 arvioi kotipuolen markkinaa",
          "Mukana rating, formi, koti-/vierasetu ja lajikohtainen boost",
        ],
      },
      {
        key: "draw",
        ...
        reasons: [
          "Tasapeli analysoidaan vain jos markkina tarjoaa tasapelikertoimen",
          "Soveltuu erityisesti jalkapalloon ja 1X2-markkinoihin",
        ],
      },
      {
        key: "away",
        ...
        modelProb: Math.min(0.97, model.away + sportBoost(match, "away")),
        reasons: [
          "Model V3 arvioi vieraspuolen markkinaa",
          "Mukana rating, formi, vierasrasitus ja lajikohtainen signaali",
        ],
      },
    ];
  }

  return finalizeRows(rows, match, bankroll, modelSummary);
}
...
function bScore(pick) {
  const edgeScore = Number(pick.edge || 0) * 100;
  const evScore = Number(pick.ev || 0) * 10;
  const oddsPenalty = Number(pick.odds || 0) > 10 ? 8 : 0;
  const modelBonus = pick.modelVersion === "v3" ? 2 : 0;
