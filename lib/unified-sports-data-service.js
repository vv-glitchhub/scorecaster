...
export async function loadUnifiedSportsData(
  ...
  if (!match.homeTeam || !match.awayTeam || !match.sportKey) {
    return {
      ...
      ledger: applyPregameEvidenceCoverage(buildUnifiedSportsDataLedger({ pick, sportsReport, now }))
    };
  }
  ...
  const ledger = applyPregameEvidenceCoverage(buildUnifiedSportsDataLedger({ pick, sportsReport, secondaryOdds, context, weather, now }));
