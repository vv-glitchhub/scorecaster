...
async function loadSystemSafety(admin) {
  const [alertsResult, incidentsResult, diagnosticsResult] = await Promise.all([
    admin.from("decision_diagnostic_alerts")
      .select("alert_type,severity,title,message,active,details,last_seen_at")
      .eq("active", true).order("last_seen_at", { ascending: false }).limit(50),
    admin.from("unified_data_incidents")
      .select("incident_type,severity,title,message,active,details,last_seen_at")
      .eq("active", true).order("last_seen_at", { ascending: false }).limit(50),
    admin.from("decision_diagnostic_snapshots")
      .select("captured_at,status,stale_rate,average_bookmakers,average_confidence,provider_health")
      .order("captured_at", { ascending: false }).limit(1).maybeSingle()
  ]);
  const error = alertsResult.error || incidentsResult.error || diagnosticsResult.error;
  if (error) throw error;
  return {
    decisionAlerts: alertsResult.data || [],
    unifiedIncidents: incidentsResult.data || [],
    diagnostics: diagnosticsResult.data || null
  };
}

async function completeUser(admin, userId, result, performance, brief, pauseReason = null) {
  const paused = result.status === "paused";
  const pauseMinutes = paused ? Math.max(60, Math.trunc(finite(result.nextCheckMinutes, performance.cooldownHours * 60))) : 0;
  const { error } = await admin.rpc("complete_autonomous_agent_user_v2", {
    ...
    p_next_check_minutes: Math.max(15, Math.min(10080, Math.trunc(finite(result.nextCheckMinutes, 180)))),
    p_health_status: result.healthStatus || performance.status || "learning",
    p_health_score: finite(result.healthScore, performance.score || 50),
    p_resolved_sample: performance.resolvedSample || 0,
    p_consecutive_losses: performance.consecutiveLosses || 0,
    p_drawdown_percent: round(finite(performance.drawdown) * 100, 4),
    p_roi: performance.roi ?? null,
    p_average_clv: performance.clv?.average ?? null,
    p_pause_minutes: pauseMinutes,
    p_pause_reason: pauseReason ? text(pauseReason, 500) : null,
    p_last_brief: brief || {}
  });
  if (error) throw error;
}
...
async function saveBrief(admin, userId, brief, now) {
  const { error } = await admin.from("autonomous_agent_daily_briefs")
    .upsert({ user_id: userId, brief_date: now.toISOString().slice(0, 10), brief }, { onConflict: "user_id,brief_date" });
  if (error) throw error;
}
...
function chooseDecisions(decisions, context, riskGovernor, performance, system, now) {
  ...
  for (const decision of decisions) {
    const evaluation = evaluateAutonomousCandidate(decision, {
      settings: { ...settings, minPriorityScore: requiredPriority },
      bankroll,
      performance,
      system,
      openEventIds: usedEvents,
      now: now.getTime()
    });
    const event = eventIdentity(decision);
    const league = leagueIdentity(decision);
    const reasons = [...evaluation.reasons];
    if (!riskGovernor.allowNewExposure) reasons.push("risk_governor_paused");
    if (!event) reasons.push("missing_event_identity");
    if (selected.length >= remainingDailyPicks) reasons.push("daily_pick_limit_reached");
    if (exposure.events.size + selected.length >= settings.maxOpenPicks) reasons.push("maximum_open_picks_reached");
    const uniqueReasons = [...new Set(reasons)];
    if (uniqueReasons.length) {
      const audit = { ...evaluation, allowed: false, reasons: uniqueReasons };
      audited.push({ decision, audit, proposedStake: 0 });
      skipped.push({ decision, reasons: uniqueReasons, audit });
      continue;
    }
    ...
    const requested = Math.max(0, finite(decision.allocatedStake || decision.suggestedStake))
      * riskGovernor.stakeMultiplier
      * finite(performance.stakeMultiplier, 1);
    ...
    if (stake < 0.01) {
      ...
      const audit = { ...evaluation, allowed: false, reasons: stakeReasons };
      audited.push({ decision, audit, proposedStake: 0 });
      skipped.push({ decision, reasons: stakeReasons, audit });
      continue;
    }

    const accepted = { ...decision, autonomousStake: stake, autonomousAudit: evaluation };
    selected.push(accepted);
    audited.push({ decision: accepted, audit: evaluation, proposedStake: stake });
    usedEvents.add(event);
    remainingTotal = Math.max(0, remainingTotal - stake);
    leagueUsed.set(league, leagueExposure + stake);
  }
  return { selected, skipped, audited, exposure, daily, dailyCap, remainingDailyPicks, requiredPriority };
}
...
function clientRef(userId, decision, now) {
  ...
  return `autonomous-v13-${day}-${digest}`;
}
...
function contextAudit(decision = {}) {
  return {
    ...
    unusedOrMissingData: safeList(decision.missingEvidence || decision.missingData || decision.unusedDataSources || []),
    ...
    autonomousV13: decision.autonomousAudit || null,
    unifiedSportsData: decision.unifiedSportsData || null
  };
}

function paperRow(userId, runId, decision, source, fixtureSource, now, riskGovernor, performance, system) {
  ...
  return {
    ...
    status: "open",
    raw_pick: {
      source: "scorecaster-autonomous-v13",
      eventId,
      ...
      agentVersion: "Autonomous-Scorecaster-V13-Integrated-Governance",
...
function auditRow(userId, runId, decision, audit, proposedStake = 0, savedBetId = null) {
  return {
    user_id: userId,
    run_id: runId,
    event_id: pickEventId(decision) || audit.eventId,
    match: pickMatch(decision),
    selection: pickSelection(decision),
    sport: pickSport(decision),
    league: pickLeague(decision),
    allowed: audit.allowed,
    reasons: safeList(audit.reasons, 20, 100),
    warnings: safeList(audit.warnings, 20, 100),
    quality_score: audit.qualityScore,
    priority_score: round(decision.priorityScore, 6),
    odds: finite(decision.odds),
    edge: finite(decision.edge),
    confidence: finite(decision.confidence),
    ...
    proposed_stake: Math.max(0, finite(proposedStake)),
...
async function saveAuditRows(admin, rows) {
  ...
  const { error } = await admin.from("autonomous_agent_decision_audit").insert(rows.slice(0, MAX_AUDIT_DECISIONS));
  if (error) throw error;
}
...
async function processUser(admin, entry, source, globalSafety, now, globalBudget) {
  ...
  try {
    ...
    const choice = chooseDecisions(governed, entry.context, riskGovernor, performance, system, now);
    const selected = choice.selected.slice(0, Math.max(0, globalBudget));
    const auditSummary = choice.skipped.slice(0, MAX_AUDIT_DECISIONS).map((item) => summaryDecision(item.decision, false, item.reasons));
    ...
    for (const decision of selected) {
      const saveResult = await saveDecision(
        admin,
        paperRow(entry.userId, runId, decision, source.source, source.fixtureSource, now, riskGovernor, performance, system)
      );
      if (saveResult.saved) {
        savedCount += 1;
        totalStake += decision.autonomousStake;
        savedIds.set(`${eventIdentity(decision)}|${normalized(pickSelection(decision))}`, saveResult.data.id);
        auditSummary.push(summaryDecision(decision, true, ["saved_paper_play"]));
      } else if (saveResult.duplicate) {
        duplicateCount += 1;
        auditSummary.push(summaryDecision(decision, false, ["duplicate_daily_decision"]));
      } else if (saveResult.riskRejected) {
        riskRejectedCount += 1;
        auditSummary.push(summaryDecision(decision, false, ["database_risk_limit"]));
      } else {
        saveErrorCount += 1;
        auditSummary.push(summaryDecision(decision, false, ["save_failed"]));
      }
    }

    const auditRows = choice.audited.map((item) => auditRow(
      entry.userId,
      runId,
      item.decision,
      item.audit,
      item.proposedStake,
      savedIds.get(`${eventIdentity(item.decision)}|${normalized(pickSelection(item.decision))}`) || null
    ));
    await saveAuditRows(admin, auditRows);

    const paused = system.status === "blocked" || performance.status === "paused" || !riskGovernor.allowNewExposure;
    const result = {
    ...
    result.nextCheckMinutes = adaptiveNextCheckMinutes({ result, settings: entry.context.settings, system, performance });
    const brief = buildAutonomousDailyBrief({ performance, system, result, audits: choice.audited.map((item) => item.audit), generatedAt: now.toISOString() });
    const summary = {
      paperOnly: true,
      agentVersion: "Autonomous-Scorecaster-V13-Integrated-Governance",
      ...
      decisions: auditSummary.slice(0, MAX_AUDIT_DECISIONS)
    };
    await finishRun(admin, runId, result, summary, now);
    await saveBrief(admin, entry.userId, brief, now);
    const pauseReason = paused ? [...(system.reasons || []), ...(performance.reasons || []), ...(riskGovernor.hardReasons || [])].join(", ") : null;
    await completeUser(admin, entry.userId, result, performance, brief, pauseReason);
    return result;
  } catch (error) {
    const failure = {
      status: "error",
      ...
      skippedCount: 0,
      totalStake: 0,
      error: text(error?.message, 500, "Autonomous Agent V13 processing failed"),
      nextCheckMinutes: 60,
...
export async function runAutonomousPaperAgentV2({ admin, origin, now = new Date() } = {}) {
  if (!admin) throw new Error("Autonomous Paper Agent V13 requires a Supabase admin client");
  if (!origin) throw new Error("Autonomous Paper Agent V13 requires a request origin");
  const startedAt = now instanceof Date ? now : new Date(now);
  if (Number.isNaN(startedAt.getTime())) throw new Error("Autonomous Paper Agent V13 requires a valid clock value");
  ...
  for (const userId of userIds) {
    try {
      ...
      else if (context.openBets.length > Math.min(MAX_OPEN_BETS_PER_USER, context.settings.maxOpenPicks)) deferred.push({ userId, context, reason: "Open paper bet limit exceeds the configured safety maximum" });
  ...
  for (const entry of selectedEntries.filter((item) => sourceFailures.has(item.sourceKey))) {
    try {
      ...
      // Completion failure is surfaced by the protected worker route without exposing credentials.
    }
  }
  ...
  return {
    ok: true,
    version: "autonomous-scorecaster-v13-integrated-governance",
    paperOnly: true,
    realMoneyBetting: false,
    ...
    limits: {
      ...
      auditRowsPerUser: MAX_AUDIT_DECISIONS,
