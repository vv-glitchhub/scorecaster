...
function maximum(values = [], digits = 4) {
  const usable = values.map(finite).filter((value) => value !== null);
  return usable.length ? round(Math.max(...usable), digits) : null;
}
...
function safeObservationUpstream(row = {}) {
  ...
  return {
    ...
    usage: safeSportsGameOddsUsageEvidence(upstream.usage)
  };
}
...
function aggregateUsageEvidence(evidence = []) {
  const usages = evidence.map((row) => row.usage).filter(Boolean);
  const bindingLimits = [...new Set(usages.flatMap((usage) => usage.bindingLimits || []))].sort();
  const intervals = {};
  for (const interval of SPORTSGAMEODDS_USAGE_INTERVALS) {
    intervals[interval] = {
      maximumObservedRequestRatio: maximum(usages.map((usage) => usage.intervals?.[interval]?.requestRatio)),
      maximumObservedEntityRatio: maximum(usages.map((usage) => usage.intervals?.[interval]?.entityRatio))
    };
  }
  return {
    observed: usages.length > 0,
    observationsCarryingUsage: usages.length,
    activeFalseObserved: usages.some((usage) => usage.isActive === false),
    bindingLimits,
    intervals,
...
function aggregateUpstreamErrors(rows = []) {
  ...
  return {
    ...
    usage: aggregateUsageEvidence(evidence)
  };
}
...
export function buildSecondaryPricingDiagnostics({ snapshots = [], providerObservations = [] } = {}) {
  ...
  return {
    ...
    semantics: {
      ...
      usageCopiesNotTreatedAsIndependentSamples: true,
