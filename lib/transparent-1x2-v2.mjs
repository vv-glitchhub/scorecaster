...
export function buildTransparent1X2V2(input = {}, configuration = {}) {
  ...
  if (profile.supplied && profile.status === "validated" && !profile.valid) {
    return {
      ok: false,
      modelVersion: TRANSPARENT_1X2_V2_VERSION,
      baselineModelVersion: baseline.modelVersion,
      reason: profile.reason === "training-cutoff-after-prediction"
        ? "challenger-profile-chronology-violation"
        : "invalid-validated-challenger-profile",
