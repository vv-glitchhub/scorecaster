...
const SOURCE_ID = "odds_api_io_veikkaus";
const SOURCE_TERMS = "https://odds-api.io/terms";
...
export function veikkausOddsConfiguration(env = process.env) {
  ...
  const enabledFlag = enabled(env.VEIKKAUS_ODDS_ENABLED);
  const commercialUseAllowed = enabled(env.VEIKKAUS_ODDS_COMMERCIAL_ALLOWED);
  const production = env.VERCEL_ENV === "production" || env.NODE_ENV === "production";
  const configured = Boolean(apiKey);
  const rightsSatisfied = !production || commercialUseAllowed;
  const active = configured && enabledFlag && rightsSatisfied;
  ...
  if (!sport || !baseGames.length) {
    return {
      ...
      state: providerState({
        ...
        enabled: true,
        commercialUseAllowed: configuration.commercialUseAllowed,
        mode: sport ? "no-events" : "unsupported-sport"
      })
    };
  }
...
export const VEIKKAUS_ODDS_SOURCE_ID = SOURCE_ID;
