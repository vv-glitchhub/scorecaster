...
function buildRiskWarnings({ allocations, bank, allocated, drawdown, exposureByLeague }) {
  ...
  if (totalExposure >= 0.12) warnings.push("Portfolio exposure is high. Consider reducing stakes.");
  if (Math.abs(drawdown) >= 0.06) warnings.push("Drawdown detected. Risk multiplier has reduced staking.");
...
function buildAllocationReason(pick, stake) {
  ...
  if (pick.decision === "BET") return "High-ranked Agent V8 pick inside bankroll and risk limits.";
