...
export async function fetchNewsForMatch({ homeTeam, awayTeam, sport, league }) {
  ...
  const rawQuery = `("${homeTeam}" AND "${awayTeam}") OR ("${homeTeam}" AND ${
    league || sport || ""
  }) OR ("${awayTeam}" AND ${league || sport || ""})`;
  ...
  try {
    ...
    const cleanRelevant = relevant.filter(
      (article) => !isBlockedUrl(article.url)
    );
    ...
    const selected = cleanRelevant.length
      ? cleanRelevant
      : cleanArticles.slice(0, 5);
    ...
    return {
      ...
      relevantCount: cleanRelevant.length,
