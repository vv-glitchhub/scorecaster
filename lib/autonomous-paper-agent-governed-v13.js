...
// The production worker runs on a 60 second serverless budget. Keep each cycle
// deliberately small; the 15 minute scheduler will pick up the next leased
// users instead of letting one request accumulate unbounded provider work.
const MAX_USERS_PER_RUN = 2;
const MAX_SPORTS_PER_USER = 6;
const MAX_SOURCE_GROUPS_PER_RUN = 2;
const MAX_INLINE_MARKET_SCAN_EVENTS = 1;
...
function pickCommenceTime(pick = {}) {
  const value = pick.commenceTime || pick.commence_time || pick.startTime || pick.start_time;
...
async function loadTopPicks(origin, sports, now = new Date()) {
  ...
  try {
    const scan = await scanAutonomousMarketUniverse({
      ...
      origin,
      now,
      maxEventsPerScan: MAX_INLINE_MARKET_SCAN_EVENTS
    });
...
function auditRow(userId, runId, decision, audit, proposedStake = 0, savedBetId = null) {
  return {
    ...
    commence_time: pickCommenceTime(decision),
    match: pickMatch(decision),
...
export async function runAutonomousPaperAgentV2({ admin, origin, now = new Date() } = {}) {
  ...
  return {
    ...
    limits: {
      ...
      inlineMarketScanEvents: MAX_INLINE_MARKET_SCAN_EVENTS,
