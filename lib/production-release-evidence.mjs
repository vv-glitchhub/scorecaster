...
function probeEvidenceMap(value = {}, pathPrefix = "/api/") {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(Object.entries(value).map(([key, entry]) => {
  ...
  }).filter(([path]) => pathPrefix === "/" ? path.startsWith("/") : path.startsWith(pathPrefix)));
}
...
function declaredProbe(item, evidenceMap) {
  const path = clean(item?.path, 180);
  const method = clean(item?.method, 12).toUpperCase();
  const expectedUnauthenticatedStatuses = Array.isArray(item?.allowedStatuses)
    ? item.allowedStatuses.map(integer).filter((status) => status !== null)
    : [];
  const evidence = evidenceMap[path] || { status: "unverified", observedAt: null, httpStatus: null, evidenceRef: null };
  const passedStatusMatches = evidence.status === "passed"
    && evidence.httpStatus !== null
    && expectedUnauthenticatedStatuses.includes(evidence.httpStatus);
  return {
    path,
    method,
    expectedUnauthenticatedStatuses,
    status: evidence.status === "passed" && !passedStatusMatches ? "unverified" : evidence.status,
...
function declaredPublicPage(pathValue, evidenceMap) {
  const path = clean(pathValue, 180);
  const evidence = evidenceMap[path] || { status: "unverified", observedAt: null, httpStatus: null, evidenceRef: null };
  const passedStatusMatches = evidence.status === "passed" && evidence.httpStatus === 200;
  return {
    path,
...
export function buildProductionReleaseEvidence({
  ...
  const publicPages = Array.isArray(manifest.publicPages) ? manifest.publicPages.map((item) => clean(item, 180)).filter((item) => item.startsWith("/")) : [];
  const requiredSecurityHeaders = Object.fromEntries(Object.entries(manifest.requiredSecurityHeaders || {}).map(([key, value]) => [clean(key, 120).toLowerCase(), clean(value, 240)]).filter(([key, value]) => key && value));
  const gateEvidence = gateEvidenceMap(manualGateEvidence);
  const workerEvidence = probeEvidenceMap(workerProbeEvidence);
  const apiEvidence = probeEvidenceMap(protectedApiProbeEvidence);
  const pageEvidence = probeEvidenceMap(publicSurfaceEvidence, "/");
  ...
  const protectedWorkerProbes = internalWorkers
    .map((item) => declaredProbe(item, workerEvidence))
    .filter((item) => item.path.startsWith("/api/"));
  const protectedApiProbes = protectedApis
    .map((item) => declaredProbe(item, apiEvidence))
    .filter((item) => item.path.startsWith("/api/"));
  const publicSurfaceProbes = publicPages.map((item) => declaredPublicPage(item, pageEvidence));
  ...
  const migrationInventory = {
    ...
    configuredMigrationCount: finite(migrationEvidence.configuredMigrationCount) ?? migrations.length,
    recordedMigrationCount: finite(migrationEvidence.recordedMigrationCount),
    verifiedAppliedCount: finite(migrationEvidence.verifiedAppliedCount),
    unverifiedCount: finite(migrationEvidence.unverifiedCount),
    unresolvedCount: finite(migrationEvidence.unresolvedCount),
    orderMatchesManifest: migrationEvidence.orderMatchesManifest === true,
    validationPassed: migrationEvidence.validationPassed === true,
    productionVerified: migrationEvidence.productionVerified === true,
    registrySchemaVersion: finite(migrationEvidence.registrySchemaVersion),
    statusFingerprint: /^[0-9a-f]{64}$/i.test(clean(migrationEvidence.statusFingerprint, 80))
      ? clean(migrationEvidence.statusFingerprint, 80).toLowerCase()
      : null,
    validationFailureCount: finite(migrationEvidence.validationFailureCount),
  ...
  const unresolvedApiProbes = protectedApiProbes.filter((probe) => probe.status !== "passed");
  const unresolvedPublicSurface = publicSurfaceProbes.filter((probe) => probe.status !== "passed");
  const blockers = [];
  ...
  if (migrationInventory.status !== "passed" || !migrationInventory.productionVerified) blockers.push("production-migrations-unverified");
  ...
  if (unresolvedApiProbes.length) blockers.push("protected-api-probes-unverified");
  if (unresolvedPublicSurface.length) blockers.push("public-surface-probes-unverified");
  ...
  const manifestIdentity = {
    ...
    protectedApiPaths: protectedApiProbes.map((probe) => `${probe.method}:${probe.path}`)
  };
  ...
  return {
    ...
    evidenceSummary: {
      ...
      migrationsVerified: migrationInventory.status === "passed" && migrationInventory.productionVerified,
      runtimeWorkerEnabled: runtimeWorker.state === "enabled",
      publicSurfaceProbesPassed: publicSurfaceProbes.length > 0 && unresolvedPublicSurface.length === 0,
      protectedWorkerProbesPassed: protectedWorkerProbes.length > 0 && unresolvedWorkerProbes.length === 0,
      protectedApiProbesPassed: protectedApiProbes.length > 0 && unresolvedApiProbes.length === 0,
    ...
    provenance: {
      ...
      migrationInventory: "config/release-readiness.json plus redacted summary derived from config/production-migration-status.json; file presence never proves production application",
      publicSurfaceProbes: "release manifest public page/header declarations plus separately supplied trusted production page probe evidence",
      workerProbes: "release manifest declarations plus separately supplied trusted production worker probe evidence",
      protectedApiProbes: "release manifest declarations plus separately supplied trusted unauthenticated protected-API probe evidence",
    ...
    safety: {
      ...
      migrationFilePresenceUsedAsProductionProof: false,
      rawMigrationEvidenceRefsIncluded: false,
