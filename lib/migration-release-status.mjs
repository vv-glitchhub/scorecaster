...
const fullyVerifiedApplied = (entry) => entry.status === "applied"
  && Boolean(entry.verifiedAt)
  && entry.verifiedByPresent
...
export function buildMigrationReleaseStatus({ manifest = {}, statusDocument = {} } = {}) {
  ...
  const verifiedAppliedCount = normalized.filter(fullyVerifiedApplied).length;
  const unresolvedCount = normalized.filter((entry) => !fullyVerifiedApplied(entry)).length;
  const allApplied = configured.length > 0
    && normalized.length === configured.length
    && verifiedAppliedCount === configured.length;
