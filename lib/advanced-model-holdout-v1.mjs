import { buildModelValidationEvidence, validationNumber, VALIDATION_REVIEW_POLICY } from "./validation-lab-v1.mjs";
...
const MIN_PROVISIONAL_SAMPLE = VALIDATION_REVIEW_POLICY.provisionalSample;
const MIN_REVIEW_SAMPLE = VALIDATION_REVIEW_POLICY.minimumPairedEvents;
...
function resultTime(result = {}) {
  ...
  const time = clean(result.time, 20);
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date) || !/^\d{2}:\d{2}(:\d{2})?(Z|[+-]\d{2}:\d{2})?$/.test(time)) return null;
  return timestamp(`${date}T${time}${/Z|[+-]\d{2}:\d{2}$/.test(time) ? "" : "Z"}`);
}

function matchResult(prediction, results = [], now) {
  const commence = timestamp(prediction.commenceTime);
  if (commence === null || commence >= now) return null;
  const candidates = results.filter((result) => result?.is_finished === true)
    .filter((result) => !result.holdoutLeagueKey || result.holdoutLeagueKey === `${prediction.sportKey}|${prediction.league}`)
    ...
    .filter((item) => item.time !== null && item.time < now && Math.abs(item.time - commence) <= 90 * 60 * 1000);
  // Identical provider duplicates are harmless; conflicting or ambiguous matches
  // require an explicit canonical mapping and must not enter the scorecard.
  const unique = new Map(candidates.map(({ result, time }) => [
    JSON.stringify([result.id || null, time, result.home_score, result.away_score]), result
  ]));
...
function normalizeSnapshotPredictions(snapshotRows = [], now) {
  const candidates = [];
  for (const snapshot of Array.isArray(snapshotRows) ? snapshotRows : []) {
    ...
    if (!capturedAt || !commenceTime || capturedTime >= commence || capturedTime > now) continue;
    const eventId = clean(snapshot.event_id ?? snapshot.eventId, 180);
    if (!eventId) continue;
    ...
    for (const model of Array.isArray(summary.shadowModels) ? summary.shadowModels : []) {
      ...
      if (timestamp(predictionHorizon) >= commence || timestamp(predictionHorizon) > capturedTime) continue;
      candidates.push({
        eventId,
  ...
  for (const prediction of candidates) {
    const identity = JSON.stringify([prediction.sportKey, prediction.league, prediction.eventId, prediction.modelId, prediction.modelVersion]);
...
function benchmarkEvidence(modelRows = [], modelBrier, modelLogLoss) {
  ...
  return {
    ...
    reviewEligible,
    positiveComparison: reviewEligible && fullComparableSample,
    skillClaimAllowed: false,
    skillClaimBlocker: "prospective-validation-and-training-provenance-required",
...
function summarizeModel(modelRows, now) {
  ...
  const result = {
    ...
    sport,
    sportKey: modelRows[0]?.sportKey || null,
    league: modelRows[0]?.league || null,
    sampleSize,
    status,
    brier,
    logLoss,
    ...
    validationEvidence: buildModelValidationEvidence(modelRows, { now }),
...
export function buildAdvancedModelHoldoutV1(snapshotRows = [], resultRows = [], { now = Date.now() } = {}) {
  const predictions = normalizeSnapshotPredictions(snapshotRows, now);
  const evaluated = [];
  const unmatched = [];
  for (const prediction of predictions) {
    const result = matchResult(prediction, resultRows, now);
    if (!result) {
      unmatched.push({ eventId: prediction.eventId, modelVersion: prediction.modelVersion, reason: "unambiguous-final-result-not-found" });
      continue;
    }
    const row = evaluatePrediction(prediction, result);
    if (row && finite(row.brier) !== null && finite(row.logLoss) !== null) evaluated.push(row);
  ...
  for (const row of evaluated) {
    const key = JSON.stringify([row.modelId, row.modelVersion, row.sport, row.sportKey, row.league]);
  ...
  return {
    ...
    validationPolicy: VALIDATION_REVIEW_POLICY,
    counts: {
