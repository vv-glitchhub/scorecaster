import { runAutonomousPaperAgentV2 as runGovernedAutonomousPaperAgentV13 } from "./autonomous-paper-agent-governed-v13.js";
import { buildSelfLearningReport } from "./agent-self-learning.mjs";
import { applyAutonomousSystemCaps } from "./autonomous-risk-governor.mjs";
import {
  buildAutonomyJournal,
  buildAutonomyState
} from "./autonomous-scorecaster-v12.mjs";

// Vercel's production function budget is 60 seconds. These caps bound the
// extra V12 safety/journal work around the governed V13 paper-agent cycle.
const PREFLIGHT_LIMIT = 4;
const RECENT_RUN_ENRICH_LIMIT = 4;
...
async function insertDeferredRun(admin, userId, context, state, now) {
  ...
  const { data, error } = await admin.from("autonomous_agent_runs")
    .insert({
      ...
      health_score: 0,
      guard_summary: { autonomyV12: state },
      next_check_minutes: cooldownHours(state) * 60,
      summary: {
        paperOnly: true,
        realMoneyBetting: false,
        agentVersion: "Autonomous-Scorecaster-V13",
...
async function runPreflight(admin, now) {
  ...
  const { data, error } = await admin.from("autonomous_agent_state")
    .select("user_id,next_check_at,lease_expires_at,last_status,paused_until")
...
export async function runAutonomousScorecasterV12({ admin, origin, now = new Date() } = {}) {
  if (!admin) throw new Error("Autonomous Scorecaster V13 requires a Supabase admin client");
  if (!origin) throw new Error("Autonomous Scorecaster V13 requires a request origin");
  const startedAt = now instanceof Date ? now : new Date(now);
  if (Number.isNaN(startedAt.getTime())) throw new Error("Autonomous Scorecaster V13 requires a valid clock value");

  const preflight = await runPreflight(admin, startedAt);
  const base = await runGovernedAutonomousPaperAgentV13({ admin, origin, now: startedAt });
  const enrichedRuns = await enrichRecentRuns(admin, startedAt, new Date());

  return {
    ...base,
    version: "autonomous-scorecaster-v13",
    ...
    v12: {
      ...
      safety: {
        ...
        persistentUtcDailyPickLimit: true,
        persistentUtcDailyStakeCap: true,
        persistentSameEventDailyDeduplication: true,
        systemMaxStakePercent: 1,
        systemMaxDailyExposurePercent: 5,
        systemMaxLeagueExposurePercent: 2.5,
    ...
    v13: {
      candidateAudit: true,
      dailyBrief: true,
      incidentCircuitBreaker: true,
      providerDisagreementGate: true,
      adaptiveCadence: true,
      emergencyStop: true,
      shadowLearningOnly: true,
      databaseHardCapsRequired: true,
