export const PROFESSIONAL_SELECTION_EVIDENCE_VERSION = "scorecaster-professional-selection-evidence-v1.2";
...
const normalizedMode = (selection = {}) => clean(
  selection.modelMode ?? selection.modelRole ?? selection.probabilityType ?? selection.modelType,
  80
).toLowerCase();

const isMarketOnlyMode = (selection = {}) => {
  const mode = normalizedMode(selection);
  return mode.includes("market") && (mode.includes("consensus") || mode.includes("benchmark") || mode.includes("implied"));
};
...
export function professionalOffers(selection = {}) {
  ...
  for (const offer of candidateOffers(selection)) {
    ...
    const currentTime = Date.parse(offer.observedAt || 0) || 0;
    const previousTime = Date.parse(previous?.observedAt || 0) || 0;
    if (!previous || offer.odds > previous.odds || (offer.odds === previous.odds && currentTime > previousTime)) {
...
function independentModelProbability(selection = {}) {
  const explicitlyIndependent = finite(
    selection.independentModelProbability ??
    selection.independent_model_probability ??
    selection.predictedProbability ??
    selection.predicted_probability
  );
  if (explicitlyIndependent !== null) return explicitlyIndependent;
  if (isMarketOnlyMode(selection)) return null;
  return finite(selection.modelProbability ?? selection.model_probability ?? selection.probability);
}

function marketBenchmarkProbability(selection = {}) {
  if (isMarketOnlyMode(selection)) {
    return finite(
      selection.marketConsensusProbability ??
      selection.consensusProbability ??
...
export function evaluateProfessionalSelection(selection = {}, preferences = {}) {
  ...
  const modelProbability = independentModelProbability(selection);
  const marketProbability = marketBenchmarkProbability(selection);
  const modelEvidenceAvailable = modelProbability !== null;
  const marketOnly = !modelEvidenceAvailable && marketProbability !== null;
  const fairOdds = finite(selection.fairOdds) ?? (modelProbability !== null && modelProbability > 0 ? rounded(1 / modelProbability) : null);
  const edge = finite(selection.edge) ?? (modelProbability !== null && marketProbability !== null ? rounded(modelProbability - marketProbability) : null);
  const ev = modelProbability !== null && offer.odds !== null && offer.odds > 1
    ? rounded(modelProbability * offer.odds - 1)
    : null;
  ...
  return {
    ...
    invariants: {
      ...
      marketConsensusNeverMislabelledAsIndependentModel: true,
