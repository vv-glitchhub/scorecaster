...
const MAX_RECOMMENDATION_OVERLAY = 20;
...
async function loadRecommendationOverlay(origin, fetchImpl) {
  const target = new URL("/api/recommendations", origin);
  target.searchParams.set("limit", String(MAX_RECOMMENDATION_OVERLAY));
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 25_000);
  try {
    ...
    return new Map(payload.recommendations.map((item) => [selectionKey(item), {
      recommendationRank: Number(item.rank || 0) || null,
      recommendationScore: Number(item.score || 0),
      recommendationNearPlay: item.intelligenceV2?.nearPlay === true,
      recommendationNearPlayGate: item.intelligenceV2?.nearPlayGate || null
    }]));
  } finally {
    clearTimeout(timeout);
  }
...
function overlayRecommendationMetadata(currentPicks, overlay) {
  if (!(overlay instanceof Map) || !overlay.size) return currentPicks;
  return currentPicks.map((pick) => ({ ...pick, ...(overlay.get(selectionKey(pick)) || {}) }));
}
...
export async function runWatchlistMonitor({
  ...
  let recommendationOverlay = new Map();
  try {
    recommendationOverlay = await loadRecommendationOverlay(origin, fetchImpl);
    currentPicks = overlayRecommendationMetadata(currentPicks, recommendationOverlay);
  } catch {
    recommendationOverlay = new Map();
  }
  ...
  return {
    ...
    recommendationOverlay: recommendationOverlay.size
  };
}
