...
function getBestH2HOutcomes(oddsData) {
  ...
  for (const bookmaker of bookmakers) {
    ...
    for (const outcome of h2h.outcomes) {
      ...
      if (!best.home && outcome.__type === "home") {
        best.home = price;
        continue;
      }

      if (!best.away && outcome.__type === "away") {
        best.away = price;
        continue;
      }

      if (!best.draw && outcome.__type === "draw") {
        best.draw = price;
        continue;
      }
    }
...
export async function getModelProbabilitiesForMatch({
  ...
  // Jos ei rating-dataa, käytä siistiä market fallbackia
