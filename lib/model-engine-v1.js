...
export function pickBestOddsForTeams(oddsRows, homeTeam, awayTeam) {
  ...
  return { bestHomeOdds, bestAwayOdds };
}
...
export function buildQuickModel({
  ...
  const homeFinalStrength =
    homeBaseStrength + homeContextAdjustment + homePlayerAdjustment;
  const awayFinalStrength =
    awayBaseStrength + awayContextAdjustment + awayPlayerAdjustment;
  ...
  const { bestHomeOdds, bestAwayOdds } = pickBestOddsForTeams(
    oddsRows,
    context?.home_team,
    context?.away_team
  );
