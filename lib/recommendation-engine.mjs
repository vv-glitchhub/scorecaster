...
} from "./recommendation-intelligence-v2.mjs";
...
function ownedEvidenceOf(pick = {}) {
  return pick.ownedDecisionEvidenceV1 || pick.ownedDecisionEvidence || {};
}

function minimumEvOddsOf(pick = {}) {
  const fairOdds = Number(pick.fairOdds || 0);
  if (!Number.isFinite(fairOdds) || fairOdds <= 1) return null;
  return Number((fairOdds * 1.03).toFixed(3));
}

function nextGateOf(pick = {}, decision = decisionOf(pick)) {
  if (decision === "PLAY") {
    return { code: "maintain-play-gates", status: "open" };
  }
  ...
  if (freshness === "stale") return { code: "fresh-data", status: "blocked" };
  if (bookmakerCount < 4) return { code: "bookmaker-coverage", status: "blocked", current: bookmakerCount, target: 4 };
  if (confidence < 0.55) return { code: "confidence", status: "blocked", current: confidence, target: 0.55 };
  if (edge < 0.02) return { code: "edge", status: "blocked", current: edge, target: 0.02 };
  if (ev < 0.03) return { code: "ev", status: "blocked", current: ev, target: 0.03, minimumEvOdds: minimumEvOddsOf(pick) };
  if (readiness !== "verified") return { code: "verified-evidence", status: "blocked", current: readiness, target: "verified" };
  return { code: "safety-recheck", status: "blocked" };
}
...
export function buildRecommendation(pick = {}, rank = null) {
  ...
  const minimumEvOdds = minimumEvOddsOf(pick);
  const odds = Number(pick.odds || 0) || null;
  const marketProbability = finite(pick.marketProbability ?? pick.consensusProbability);
  const ownedEvidence = ownedEvidenceOf(pick);
  const independentModelProbability = finite(
    pick.independentModelProbability ?? ownedEvidence?.probability
  );
  const recommendation = {
    version: "scorecaster-recommendation-v1.1",
    rank,
    decision,
    ...
    sportKey: pick.sportKey || pick.sport || null,
    sportTitle: pick.sportTitle || null,
    ...
    minimumEvOdds,
    evPriceGateOpen: Boolean(odds && minimumEvOdds && odds >= minimumEvOdds),
    priceGapToMinimumEvOdds: odds && minimumEvOdds ? Number((odds - minimumEvOdds).toFixed(3)) : null,
    marketProbability,
    independentModelProbability,
    ownedModelAvailable: ownedEvidence?.applicable === true && independentModelProbability !== null,
    ownedModelQualified: ownedEvidence?.qualified === true,
    ownedModelSupportsSelection: ownedEvidence?.supportsSelection === true,
    ownedModelStrongConflict: ownedEvidence?.strongConflict === true,
    ownedModelProbabilityDelta: finite(ownedEvidence?.probabilityDelta),
    ownedModelMarketMapped: ownedEvidence?.marketMapped === true,
    ...
    kelly: finite(pick.kelly),
    adjustedKelly: finite(pick.adjustedKelly),
    suggestedStake: finite(pick.suggestedStake),
    freshness: pick.freshnessLabel || pick.dataQuality?.freshness || "unknown",
    readiness: readinessOf(pick),
    nextGate: nextGateOf(pick, decision),
  ...
  return {
    ...recommendation,
    intelligenceV2: enrichRecommendationIntelligence(recommendation)
  };
}
...
export function buildRecommendationFeed(picks = [], { limit = 8 } = {}) {
  ...
  const nearPlay = recommendations.filter((item) => item.intelligenceV2?.nearPlay === true);
  const opportunityRadar = buildOpportunityRadar(recommendations, { limit: Math.min(12, recommendations.length || 1) });
  const leagueReadiness = buildCurrentWindowLeagueReadiness(recommendations);

  return {
    version: "scorecaster-recommendation-feed-v2.1",
    generatedAt: new Date().toISOString(),
    paperOnly: true,
    realMoneyActionAvailable: false,
    topRecommendation: plays[0] || cautions[0] || null,
    hasPlayablePick: plays.length > 0,
    counts: { PLAY: plays.length, CAUTION: cautions.length, SKIP: skips.length, NEAR_PLAY: nearPlay.length },
    nearPlay,
    opportunityRadar,
    leagueReadiness,
    recommendations
