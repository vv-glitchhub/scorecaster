...
const PASSIVE_PROVIDER_MODES = new Set([
  "not_configured",
  "not_verified",
  "not_yet_available",
  "unsupported_league",
...
const PROVIDER_FAILURE_PRIORITY = [
...
function modeSummary(rows = []) {
  ...
  for (const row of rows) {
    const mode = clean(row?.mode || "unknown", 60).toLowerCase() || "unknown";
...
function primaryFailureMode(rows = []) {
  const failures = rows.filter((row) => row?.ok !== true);
  const modes = modeSummary(failures);
  for (const preferred of PROVIDER_FAILURE_PRIORITY) {
    if (modes.some((item) => item.mode === preferred)) return preferred;
  }
  return modes[0]?.mode || null;
}
...
export function summarizeProviderQuality(observations = []) {
  ...
  return [...groups.entries()].map(([provider, rows]) => {
    ...
    const failureRows = rows.filter((row) => row.ok !== true);
    ...
    const failureModes = modeSummary(failureRows);
    const passiveOnly = failureRows.length > 0 && failureRows.every((row) => PASSIVE_PROVIDER_MODES.has(clean(row?.mode, 60).toLowerCase()));
    const primaryFailure = primaryFailureMode(rows);
    return {
      provider,
      family: rows[0]?.family || "other",
      samples: rows.length,
      availableSamples: available,
      failedSamples: failureRows.length,
      ...
      status: passiveOnly && available === 0 ? "not_configured" : available === rows.length ? "healthy" : available === 0 ? "offline" : "degraded",
      primaryFailureMode: primaryFailure,
      failureModes,
      incidentEligible: failureRows.some((row) => !PASSIVE_PROVIDER_MODES.has(clean(row?.mode, 60).toLowerCase()))
    };
...
export function evaluateUnifiedDataIncidents(currentSnapshots = [], providerQuality = []) {
  ...
  for (const provider of providerQuality) {
    if (provider.samples >= 4 && provider.availabilityRate < 0.5 && provider.incidentEligible !== false) {
      const mode = provider.primaryFailureMode;
      const descriptor = mode === "rate_limited"
        ? { title: `${provider.provider} rate limit is active`, reason: "rate limiting" }
        : mode === "quota_exhausted"
          ? { title: `${provider.provider} quota is exhausted`, reason: "quota exhaustion" }
          : mode === "subscription_unavailable"
            ? { title: `${provider.provider} subscription coverage is unavailable`, reason: "subscription coverage" }
            : mode === "timeout"
              ? { title: `${provider.provider} is timing out`, reason: "timeouts" }
              : mode === "fetch_error"
                ? { title: `${provider.provider} fetches are failing`, reason: "fetch failures" }
                : mode === "api_error"
                  ? { title: `${provider.provider} API is failing`, reason: "API errors" }
                  : { title: `${provider.provider} is degraded`, reason: "provider failures" };
      incidents.push(incident({
        ...
        message: `${provider.provider} availability is ${(provider.availabilityRate * 100).toFixed(0)}% across ${provider.samples} recent observations; primary blocker: ${descriptor.reason}.`,
