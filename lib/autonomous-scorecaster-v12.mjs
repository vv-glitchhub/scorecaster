...
export function buildAutonomyState({ history = [], decisions = [], modelLab = null, bankroll = {}, openBets = [], now = new Date() } = {}) {
  ...
  if (historySummary.recent30.bankrollImpact !== null && historySummary.recent30.bankrollImpact <= -0.06) {
    if (sinceLastSettlement !== null && sinceLastSettlement < 24) blockers.push("rolling_loss_limit");
    else warnings.push("rolling_loss_recovery");
  }
  ...
  return {
    ...
    resumeConditions: blockers.map((blocker) => ({
      ...
      condition: blocker === "critical_model_drift" ? "Model drift must return below critical."
        : blocker === "rolling_loss_limit" ? "The 24-hour rolling-loss cooldown must complete before a guarded reassessment."
...
export function applyAutonomyPolicy(decisions = [], autonomyState = null) {
  ...
  return (Array.isArray(decisions) ? decisions : []).map((decision) => {
    ...
    if (decision.decision === "PLAY" && acceptedPlayCount >= Math.max(0, finite(state.pickCap))) blockers.push("autonomy:mode_pick_cap");
    ...
    const multiplier = round(finite(state.stakeMultiplier) * evidenceMultiplier * coverageMultiplier, 4);
    ...
    if (decision.decision === "PLAY" && !blocked && nextStake > 0) acceptedPlayCount += 1;
