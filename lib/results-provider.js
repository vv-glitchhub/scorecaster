...
const THESPORTSDB_FREE_V1_KEY = "123";
const cache = new Map();
...
export function resolveSportsDbLeagueKey({ sportKey, league } = {}) {
  ...
  if (normalized.includes("SERIEA")) return "SERIEA";
  if (normalized.includes("BUNDESLIGA")) return "BUNDESLIGA";
  if (normalized.includes("LIGUE1")) return "LIGUE1";
...
function safeTeamDepthRow(team, before, after, provider = null) {
  return {
    ...
    premiumRequired: provider?.premiumRequired === true,
    networkRequestMade: provider?.networkRequestMade === true
  };
}
...
async function enrichWithTeamHistory(basePayload, {
  ...
  const premiumBlocked = resolved.some((row) => row.result?.mode === "premium_key_required");
...
async function loadBaseLeagueResults({ sportKey, league, now = Date.now(), fetchImpl = fetch } = {}) {
  ...
  const apiKey = clean(process.env.THESPORTSDB_API_KEY || THESPORTSDB_FREE_V1_KEY, 80);
...
export const RESULTS_PROVIDER_POLICY = Object.freeze({
  ...
  freeV1Key: THESPORTSDB_FREE_V1_KEY,
  teamHistoryFallback: "thesportsdb-v2-premium-previous-team",
  teamHistoryPremiumRequired: true,
