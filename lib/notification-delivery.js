...
async function sendClaimedDeliveries(admin, fetchImpl, now) {
  ...
  if (!ready.length) return { claimed: claims.length, ticketed: 0, retried: 0, failed: skipped };
  ...
  try {
    ...
    return { claimed: claims.length, ticketed: 0, retried: ready.length, failed: skipped };
  }
  ...
  if (!response.ok || !Array.isArray(response.payload?.data)) {
    ...
    return {
      ...
      failed: skipped + (transient ? 0 : ready.length)
    };
  }
...
async function checkReceipts(admin, fetchImpl, now) {
  ...
  if (error) throw error;
  if (!(rows || []).length) return { checked: 0, delivered: 0, retried: 0, failed: 0, pending: 0 };

  let response;
  try {
    response = await expoRequest(fetchImpl, EXPO_RECEIPTS_URL, { ids: rows.map((row) => row.expo_ticket_id) });
  } catch {
    return { checked: 0, delivered: 0, retried: 0, failed: 0, pending: rows.length };
  }
  ...
  return { checked, delivered, retried, failed, pending };
}
