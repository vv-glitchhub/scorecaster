...
function fullEventConsensus(consensusPrices = []) {
  return consensusPrices
    .map((item) => ({
      selection: item.selection,
      probability: Number(item.consensusProbability),
      bookmakerCount: Number(item.bookmakerCount || 0),
      latestUpdate: item.latestUpdate || null
    }))
    .filter((item) => item.selection && Number.isFinite(item.probability) && item.probability > 0 && item.probability < 1);
}
...
export function createScorecasterPick({
  ...
  const eventConsensusDistribution = fullEventConsensus(consensusPrices);
