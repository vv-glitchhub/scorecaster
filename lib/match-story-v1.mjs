...
function normalizedFreshness(bet) {
  const value = String(bet?.freshnessLabel || bet?.dataQuality?.freshness || "").toLowerCase();
  return ["fresh", "aging", "stale"].includes(value) ? value : "unknown";
}
...
export function buildMatchStoryV1(bet = {}) {
  ...
  const freshness = normalizedFreshness(bet);
