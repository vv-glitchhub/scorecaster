...
export function compareTransparent1X2EvaluationPackages({ baseline = {}, challenger = {} } = {}) {
  ...
  if (!base.ok) failures.push("baseline-package-not-valid");
  if (!challenge.ok) failures.push("challenger-package-not-valid");
  if (!base.packageId) failures.push("baseline-package-id-missing");
  if (!challenge.packageId) failures.push("challenger-package-id-missing");
  ...
  if (base.predictionFingerprint && challenge.predictionFingerprint && base.predictionFingerprint === challenge.predictionFingerprint) failures.push("identical-prediction-evidence");

  const marketIdentityMatches = base.marketBenchmark.samples === challenge.marketBenchmark.samples
  ...
  const foldsAligned = fingerprint(baseFoldIdentity) === fingerprint(challengerFoldIdentity);
