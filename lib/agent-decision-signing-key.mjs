...
const SAFE_SIGNING_SOURCES = new Set(["environment", "supabase-vault"]);
...
function safeSigningSource(value, fallback = "environment") {
  const source = String(value || "").trim();
  return SAFE_SIGNING_SOURCES.has(source) ? source : fallback;
}
...
export async function resolveAgentDecisionSigningKey(options = {}) {
  const explicitEnvKey = Object.prototype.hasOwnProperty.call(options, "envKey");
  const envKey = explicitEnvKey ? options.envKey : process.env.AGENT_DECISION_SIGNING_KEY;
  const envSource = explicitEnvKey ? options.envSource : process.env.AGENT_DECISION_SIGNING_KEY_SOURCE;
  const admin = Object.prototype.hasOwnProperty.call(options, "admin") ? options.admin : undefined;
  const now = Object.prototype.hasOwnProperty.call(options, "now") ? options.now : Date.now();
  const useCache = Object.prototype.hasOwnProperty.call(options, "useCache") ? options.useCache : true;
  ...
  if (environmentKey) {
    return {
      configured: true,
      key: environmentKey,
      source: safeSigningSource(envSource, "environment")
    };
  }
...
export async function hydrateAgentDecisionSigningEnvironment(options = {}) {
  const existing = validKey(process.env.AGENT_DECISION_SIGNING_KEY);
  if (existing) {
    process.env.AGENT_DECISION_SIGNING_KEY_SOURCE = safeSigningSource(
      process.env.AGENT_DECISION_SIGNING_KEY_SOURCE,
      "environment"
    );
    return {
      configured: true,
  ...
  const resolved = await resolveAgentDecisionSigningKey({ ...options, envKey: "" });
  if (!resolved.configured || !resolved.key) {
    return { configured: false, source: "unconfigured", secretValueIncluded: false };
  }

  process.env.AGENT_DECISION_SIGNING_KEY = resolved.key;
  process.env.AGENT_DECISION_SIGNING_KEY_SOURCE = resolved.source;
  return { configured: true, source: resolved.source, secretValueIncluded: false };
}
