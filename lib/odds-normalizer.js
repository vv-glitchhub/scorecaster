...
function setBest(bestOdds, key, price, bookmaker) {
      ...
      if (market.key === "h2h") {
        for (const outcome of market.outcomes || []) {
          if (outcome.name === event.home_team) {
            setBest(bestOdds, "home", outcome.price, bookName);
          } else if (outcome.name === event.away_team) {
            setBest(bestOdds, "away", outcome.price, bookName);
          } else if (String(outcome.name).toLowerCase() === "draw") {
            setBest(bestOdds, "draw", outcome.price, bookName);
          }
        }
      }
      ...
      if (market.key === "totals") {
        for (const outcome of market.outcomes || []) {
          ...
          if (String(outcome.name).toLowerCase() === "over") {
            setBest(bestOdds, "over", outcome.price, bookName);
          }

          if (String(outcome.name).toLowerCase() === "under") {
            setBest(bestOdds, "under", outcome.price, bookName);
          }
        }
      }
      ...
      if (market.key === "spreads") {
        for (const outcome of market.outcomes || []) {
          if (outcome.name === event.home_team) {
            ...
            setBest(bestOdds, "spreadHome", outcome.price, bookName);
          }

          if (outcome.name === event.away_team) {
            bestOdds.spreadPointAway = outcome.point;
            setBest(bestOdds, "spreadAway", outcome.price, bookName);
          }
        }
      }
    }
