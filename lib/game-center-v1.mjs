...
export function normalizeGameCenterDecision(value) {
  ...
  return ["PLAY", "CAUTION", "SKIP", "UNREVIEWED"].includes(decision) ? decision : "CAUTION";
}

export function buildGameCenterEvents(picks = [], fixtures = []) {
  const events = new Map();
  for (const fixture of fixtures) {
    if (!fixture?.id || fixture.fixtureVerifiedByProvider !== true) continue;
    events.set(fixture.id, { ...fixture, selections: [] });
  }
  ...
  return [...events.values()]
    .map((event) => ({
      ...
      decision: event.selections.length ? normalizeGameCenterDecision(event.selections[0]?.productDecision || event.selections[0]?.decision) : "UNREVIEWED"
    }))
    .sort((left, right) => {
...
export function summarizeGameCenter(events = []) {
  return (Array.isArray(events) ? events : []).reduce((summary, event) => {
    ...
    summary[decision.toLowerCase()] = (summary[decision.toLowerCase()] || 0) + 1;
    return summary;
