...
export function buildMlbPitchingOffenseShadowV1(pick = {}, observations = [], { now = Date.now() } = {}) {
  ...
  if (reasons.length) {
    return {
      ...
      inputSummary: {
        ...
        optionalParkAdjustmentPresent: park !== null
      },
  ...
  const parkScore = park === null ? 0 : clamp(park, -1.5, 1.5) * 0.08;

  const homeMatchup = homeLineup + awayStarterVulnerability - awayBullpen + parkScore;
  const awayMatchup = awayLineup + homeStarterVulnerability - homeBullpen + parkScore;
  ...
  return {
    ...
    formula: {
      ...
      matchupScore: "lineupStrengthZ + opponentStarterVulnerabilityZ - opponentBullpenDepthZ + optional bounded park score",
    ...
    uncertainty: {
      ...
      optionalParkAdjustmentMissing: park === null,
