...
function transportFlags({ usageRequestMade = false, eventRequestMade = false } = {}) {
  return {
    usageRequestMade: Boolean(usageRequestMade),
    eventRequestMade: Boolean(eventRequestMade)
  };
}
...
export async function fetchSportsGameOddsForMatch(match = {}, { preflightUsage = false } = {}) {
  ...
  if (preflightUsage) {
    ...
    if (quota.blocked) {
      return {
        ...
        ...transportFlags({ usageRequestMade: true, eventRequestMade: false }),
        retrievedAt,
        data: null
      };
    }
  ...
  const eventRequestMade = true;
  if (!upstream.ok) {
    const usageAfterRateLimit = upstream.evidence.errorCategory === "rate_limited";
    return {
      ok: false,
      source: "sportsgameodds",
      ...
      ...transportFlags({ usageRequestMade: usageRequestMade || usageAfterRateLimit, eventRequestMade }),
      retrievedAt,
      data: null
    };
  }
  ...
  if (!matchResult) {
    return {
      ...
      quotaPreflightBlocked: false,
      ...transportFlags({ usageRequestMade, eventRequestMade }),
      data: null
    };
  }
