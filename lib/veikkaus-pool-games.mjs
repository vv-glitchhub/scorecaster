...
export const VEIKKAUS_GAME_FAMILIES = Object.freeze({
  ...
  MONIVETO: "pari_mutuel_multi_score_pool",
...
export function classifyVeikkausGame(name) {
  ...
  if (normalized.includes("moniveto")) return VEIKKAUS_GAME_FAMILIES.MONIVETO;
...
export function calculateSupertriplaRowsFromOverlapCounts({
  firstCount,
  secondCount,
  thirdCount,
  firstSecondOverlap = 0,
  firstThirdOverlap = 0,
  secondThirdOverlap = 0,
  tripleOverlap = 0,
}) {
  const first = assertNonNegativeInteger(firstCount, "firstCount");
  const second = assertNonNegativeInteger(secondCount, "secondCount");
  const third = assertNonNegativeInteger(thirdCount, "thirdCount");
  if (first < 1 || second < 1 || third < 1) throw new Error("position counts must be positive integers");

  const ab = assertNonNegativeInteger(firstSecondOverlap, "firstSecondOverlap");
  const ac = assertNonNegativeInteger(firstThirdOverlap, "firstThirdOverlap");
  const bc = assertNonNegativeInteger(secondThirdOverlap, "secondThirdOverlap");
  const abc = assertNonNegativeInteger(tripleOverlap, "tripleOverlap");

  if (ab > Math.min(first, second) || ac > Math.min(first, third) || bc > Math.min(second, third)) {
    throw new Error("pair overlap cannot exceed either position count");
  }
  if (abc > Math.min(ab, ac, bc)) throw new Error("tripleOverlap cannot exceed any pair overlap");

  const rows = (first * second * third) - (ab * third) - (ac * second) - (bc * first) + (2 * abc);
  if (rows < 0) throw new Error("overlap counts describe an impossible negative system size");
  return rows;
}
