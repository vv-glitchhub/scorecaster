...
export function calculateExpectedDecisionValue({ chosenValue, alternatives = [], higherIsBetter = true } = {}) {
  ...
  const candidates = (alternatives || []).map(finite).filter((value) => value !== null);
...
export function buildGolfProximityProfile(observations = [], options = {}) {
  const thresholds = (options.thresholdsMeters || [1, 3, 5, 10]).map(finite).filter((value) => value !== null && value >= 0);
  const rows = new Map();
