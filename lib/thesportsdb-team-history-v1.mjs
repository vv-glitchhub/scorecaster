...
function premiumCandidate(apiKey) {
  const key = clean(apiKey, 120);
  return Boolean(key) && key !== FREE_V1_KEY;
}
...
function eventTimestamp(event = {}) {
  const direct = Date.parse(String(event?.raw?.strTimestamp || event?.commence_time || event?.commenceTime || ""));
  if (Number.isFinite(direct)) return direct;
  const date = clean(event.date, 20);
  const time = clean(event.time, 40) || "00:00:00";
  if (!date) return null;
  const timezoneSuffix = /[zZ]|[+-]\d\d:?\d\d$/.test(time) ? "" : "Z";
  const parsed = Date.parse(`${date}T${time}${timezoneSuffix}`);
...
export async function fetchSportsDbTeamHistory({
  ...
  existingResults = [],
  now = Date.now(),
  apiKey = clean(process.env.THESPORTSDB_API_KEY || FREE_V1_KEY, 120),
  fetchImpl = fetch
} = {}) {
  ...
  const canUseV2 = premiumCandidate(apiKey);
  if (!canUseV2) {
    return {
      ok: true,
      source: "thesportsdb-v2-team-history",
      mode: "premium_key_required",
      team: safeTeam,
      premiumRequired: true,
      freeV1Key: FREE_V1_KEY,
  ...
  const cacheId = `premium:${safeTeam.toLowerCase()}`;
...
export const SPORTSDATA_TEAM_HISTORY_POLICY = Object.freeze({
  ...
  freeV1Key: FREE_V1_KEY,
  v2RequiresPremiumCandidate: true,
  freeKeySkipsV2Network: true,
  auth: "X-API-KEY",
