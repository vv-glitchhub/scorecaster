...
export default function TrackingPage() {
  ...
  const portfolioRequest = useRef(null);
  ...
  const loadPortfolio = useCallback(async ({ silent = false } = {}) => {
    ...
    try {
      const betsResponse = await fetch("/api/cloud/bets", { cache: "no-store", signal: AbortSignal.any([controller.signal, AbortSignal.timeout(20000)]) });
      const payload = await payloadOf(betsResponse);
      if (controller.signal.aborted) return;
      if (betsResponse.status === 401) {
        setStorageMode("local"); setBets(localBets); setMonitor(null);
        setClosingDrafts(Object.fromEntries(localBets.map(bet => [bet.id, String(bet.closingOdds ?? "")])));
        return;
      }
      if (!betsResponse.ok || !payload.ok || !Array.isArray(payload.data)) throw new Error("Cloud history unavailable");
      const cloudBets = payload.data.map(normalizeCloudBet);
      setStorageMode("cloud"); setBets(cloudBets);
      setClosingDrafts(Object.fromEntries(cloudBets.map(bet => [bet.id, String(bet.closingOdds ?? "")])));
      // Optional monitoring must not delay access to the user's history.
      void fetchJson("/api/cloud/settlement-monitor", { signal: controller.signal, timeoutMs: 15000 }).then(
        monitorPayload => { if (!controller.signal.aborted) setMonitor(monitorPayload); },
        () => { if (!controller.signal.aborted) setMonitor(null); }
      );
    } catch {
      if (controller.signal.aborted) return;
      setStorageMode("unavailable"); setMonitor(null);
      setError(tr({ fi: "Pilvihistoriaa ei saatu ladattua. Aiemmin näkyneet tiedot säilyvät, mutta niitä ei voi muokata ennen uutta onnistunutta latausta.", en: "Cloud history could not be loaded. Previously shown data is kept, but editing is disabled until it loads successfully.", es: "No se pudo cargar el historial. Los datos anteriores se conservan, pero no se pueden editar hasta recargarlos." }));
    } finally { if (!controller.signal.aborted) setLoading(false); }
  }, [tr]);
  useEffect(() => { void loadPortfolio(); return () => portfolioRequest.current?.abort(); }, [loadPortfolio]);
  function localStorageError() { setError(tr({ fi: "Tallennus tähän selaimeen epäonnistui. Tarkista selaimen tallennustilan asetukset.", en: "Saving in this browser failed. Check its storage settings.", es: "No se pudo guardar en este navegador. Revisa los ajustes de almacenamiento." })); }

  async function settleBet(id, result, closingOnly = false) {
    if (busy || loading || !["cloud", "local"].includes(storageMode)) return;
    ...
    if (storageMode === "local") {
      try { settleTrackedBet(id, result, { closingOdds: closing.value ?? "" }); setBets(getSafeLocalBets()); setError(""); setMessage(tr({ fi: "Tallennettu tähän selaimeen.", en: "Saved in this browser.", es: "Guardado en este navegador." })); }
      catch { localStorageError(); }
      return;
    }
    const previous = bets.find(bet => bet.id === id);
    const status = closingOnly && previous?.cloudStatus ? previous.cloudStatus : result === "win" ? "won" : result === "loss" ? "lost" : result === "push" ? "push" : "open";
    setBusy(id); setMessage(""); setError("");
    try {
      const response = await fetch("/api/cloud/bets", {
        method: "PATCH", signal: AbortSignal.timeout(30000),
  ...
  async function removeBet(id) {
    ...
    if (storageMode === "local") {
      try { deleteTrackedBet(id); const localBets = getSafeLocalBets(); setBets(localBets); setLocalCount(localBets.length); } catch { localStorageError(); }
      return;
    }
    setBusy(id); setMessage(""); setError("");
    try {
      const response = await fetch("/api/cloud/bets", { method: "DELETE", signal: AbortSignal.timeout(30000), headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ids: [id] }) });
      const payload = await payloadOf(response);
  ...
  function clearAll() {
    ...
    try { clearTrackedBets(); setBets([]); setLocalCount(0); setClosingDrafts({}); } catch { localStorageError(); }
  }
  ...
  async function checkResults() {
    ...
    try {
      const response = await fetch("/api/cloud/bets/settle", { method: "POST", signal: AbortSignal.timeout(55000) });
      const payload = await payloadOf(response);
  ...
  const storageLabel = storageMode === "cloud" ? tr({ fi: "suojattu käyttäjätili", en: "protected user account", es: "cuenta protegida" }) : storageMode === "local" ? tr({ fi: "tämä selain", en: "this browser", es: "este navegador" }) : storageMode === "unavailable" ? tr({ fi: "yhteysvirhe", en: "connection error", es: "error de conexión" }) : tr({ fi: "tarkistetaan", en: "checking", es: "comprobando" });
  ...
  return (
    <div className="space-y-7">
      ...
      {storageMode === "local" ? <div className="rounded-2xl border border-amber-300/25 bg-amber-300/10 p-4 text-sm leading-6 text-[var(--sc-text)]">{tr({ fi: "Näytetään tämän laitteen paikallinen historia. Kirjaudu, jotta käyttäjätilin kohteet näkyvät kaikilla laitteilla.", en: "Showing this device's local history. Sign in to access account-backed picks on every device.", es: "Se muestra el historial local. Inicia sesión para acceder a los pronósticos de tu cuenta." })} <Link href={loginHref("/tracking")} className="font-black underline">{tr({ fi: "Kirjaudu", en: "Sign in", es: "Iniciar sesión" })}</Link></div> : null}
      {storageMode === "cloud" && localCount > 0 ? <div className="flex flex-col gap-3 rounded-2xl border border-sky-300/25 bg-sky-300/10 p-4 text-sm text-[var(--sc-text)] sm:flex-row sm:items-center sm:justify-between"><span>{tr({ fi: `Tällä laitteella on lisäksi ${localCount} paikallista kohdetta.`, en: `This device also has ${localCount} local picks.`, es: `Este dispositivo también tiene ${localCount} pronósticos locales.` })}</span><button type="button" onClick={() => void syncLocalHistory()} disabled={busy !== ""} className="sc-button-secondary shrink-0 disabled:opacity-50">{busy === "sync-local" ? tr({ fi: "Siirretään…", en: "Syncing…", es: "Sincronizando…" }) : tr({ fi: "Siirrä pilveen", en: "Sync to cloud", es: "Sincronizar" })}</button></div> : null}
      <button type="button" onClick={() => void loadPortfolio()} disabled={loading || busy !== ""} className="sc-button-secondary disabled:opacity-50">{loading ? tr({ fi: "Ladataan historiaa…", en: "Loading history…", es: "Cargando historial…" }) : tr({ fi: "Päivitä historia", en: "Refresh history", es: "Actualizar historial" })}</button>
      {error ? <div role="alert" className="rounded-2xl border border-rose-300/25 bg-rose-300/10 p-4 text-sm text-[var(--sc-text)]">{error}</div> : null}
      {message ? <div aria-live="polite" className="rounded-2xl border border-emerald-300/25 bg-emerald-300/10 p-4 text-sm text-[var(--sc-text)]">{message}</div> : null}
      ...
      {storageMode === "cloud" && monitor ? <section className="rounded-2xl border border-[var(--sc-border)] bg-white/[0.035] p-4 text-sm leading-6 text-[var(--sc-text-secondary)]"><strong className="text-[var(--sc-text)]">{tr({ fi: "Automaattinen tulosseuranta", en: "Automatic result monitoring", es: "Seguimiento automático" })}: </strong>{monitor.monitorActive ? tr({ fi: "aktiivinen", en: "active", es: "activo" }) : tr({ fi: "ei aktiivinen", en: "not active", es: "inactivo" })}. {tr({ fi: "Viimeisin ajo", en: "Latest run", es: "Última ejecución" })}: {monitor.state?.last_completed_at ? new Date(monitor.state.last_completed_at).toLocaleString(locale) : "–"}. {tr({ fi: "Seuranta ei aseta vetoja eikä keksi puuttuvaa tulosta.", en: "Monitoring never places bets or invents missing results.", es: "El seguimiento no apuesta ni inventa resultados." })}</section> : null}

      <details className="rounded-3xl border border-[var(--sc-border)] bg-white/[0.035] p-5"><summary className="cursor-pointer font-black text-[var(--sc-text)]">{tr({ fi: "Näytä edistyneet mittarit", en: "Show advanced metrics", es: "Mostrar métricas avanzadas" })}</summary><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><MetricTile label={tr({ fi: "Keskimääräinen edge", en: "Average edge", es: "Ventaja media" })} value={formatPercent(stats.averageEdge)} tone="green" /><MetricTile label={tr({ fi: "Keskimääräinen EV", en: "Average EV", es: "EV medio" })} value={formatPercent(stats.averageEV)} tone="blue" /><MetricTile label={tr({ fi: "Keskimääräinen kerroin", en: "Average odds", es: "Cuota media" })} value={stats.averageOdds.toFixed(2)} /><MetricTile label={tr({ fi: "Nykyinen putki", en: "Current streak", es: "Racha actual" })} value={stats.currentStreak} tone="purple" /><MetricTile label={tr({ fi: "Voitot", en: "Wins", es: "Victorias" })} value={stats.wins} tone="green" /><MetricTile label={tr({ fi: "Tappiot", en: "Losses", es: "Derrotas" })} value={stats.losses} tone="red" /><MetricTile label={tr({ fi: "Palautukset", en: "Pushes", es: "Nulos" })} value={stats.pushes} tone="blue" /><MetricTile label={tr({ fi: "Avoimet", en: "Open", es: "Abiertos" })} value={stats.openBets} tone="yellow" /></div></details>
      ...
      <section>
        ...
        <div className="mb-5 grid gap-3 rounded-3xl border border-[var(--sc-border)] bg-white/[0.035] p-4 md:grid-cols-2">
          <label className="text-sm font-bold text-[var(--sc-text-secondary)]">{tr({ fi: "Näytä", en: "Show", es: "Mostrar" })}<select aria-label={tr({ fi: "Suodata paperikohteita", en: "Filter paper picks", es: "Filtrar pronósticos" })} value={filter} onChange={(event) => setFilter(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--sc-border)] bg-[var(--sc-surface)] px-4 py-3 text-[var(--sc-text)]"><option value="all">{tr({ fi: "Kaikki kohteet", en: "All picks", es: "Todos" })}</option><option value="open">{tr({ fi: "Avoimet", en: "Open", es: "Abiertos" })}</option><option value="settled">{tr({ fi: "Ratkaistut", en: "Settled", es: "Resueltos" })}</option><option value="wins">{tr({ fi: "Voitot", en: "Wins", es: "Victorias" })}</option><option value="losses">{tr({ fi: "Tappiot", en: "Losses", es: "Derrotas" })}</option><option value="pushes">{tr({ fi: "Palautukset", en: "Pushes", es: "Nulos" })}</option></select></label>
          <label className="text-sm font-bold text-[var(--sc-text-secondary)]">{tr({ fi: "Järjestys", en: "Order", es: "Orden" })}<select aria-label={tr({ fi: "Järjestä paperikohteet", en: "Sort paper picks", es: "Ordenar pronósticos" })} value={sortOrder} onChange={(event) => setSortOrder(event.target.value)} className="mt-2 w-full rounded-xl border border-[var(--sc-border)] bg-[var(--sc-surface)] px-4 py-3 text-[var(--sc-text)]"><option value="newest">{tr({ fi: "Uusin ensin", en: "Newest first", es: "Más recientes primero" })}</option><option value="oldest">{tr({ fi: "Vanhin ensin", en: "Oldest first", es: "Más antiguos primero" })}</option></select></label>
        </div>

        {loading ? <div className="h-48 animate-pulse rounded-3xl border border-[var(--sc-border)] bg-white/[0.035]" /> : storageMode === "unavailable" && !bets.length ? null : filteredBets.length === 0 ? <EmptyState title={tr({ fi: "Tällä suodattimella ei ole kohteita", en: "No picks match this filter", es: "No hay pronósticos con este filtro" })} description={tr({ fi: "Avaa varmennettu ottelu, valitse kohde ja tallenna se paperiseurantaan.", en: "Open a verified event, choose a selection and save it to paper tracking.", es: "Abre un evento verificado, elige una selección y guárdala." })} actionHref="/events" actionLabel={tr({ fi: "Avaa ottelut", en: "Open events", es: "Abrir eventos" })} /> : (
          <div className="space-y-4">
            {filteredBets.map((bet) => {
              const profit = calculateProfitLoss({ stake: bet.stake, odds: bet.odds, result: bet.result });
              const closingOdds = Number(String(bet.closingOdds ?? "").replace(",", "."));
              ...
              const itemBusy = busy !== "" || loading || storageMode === "unavailable";
              return <article key={bet.id} className="rounded-3xl border border-[var(--sc-border)] bg-white/[0.035] p-5 shadow-xl">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-3 py-1 text-xs font-black ${resultStyle(bet.result)}`}>{resultLabel(bet.result)}</span><span className="text-xs text-[var(--sc-muted)]">{bet.createdAt ? new Date(bet.createdAt).toLocaleString(locale) : tr({ fi: "Päivämäärä puuttuu", en: "Date missing", es: "Falta la fecha" })}</span></div><h2 className="mt-3 text-xl font-black tracking-tight text-[var(--sc-text)] md:text-2xl">{bet.match}</h2><div className="mt-2 text-[var(--sc-text-secondary)]"><strong>{bet.selection}</strong> @ {bet.odds}</div></div><MetricTile label={t("term.paperStake")} value={money(bet.stake)} tone="purple" /></div>
                <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5"><MetricTile label={t("term.edge")} value={formatPercent(bet.edge)} tone="green" /><MetricTile label={t("term.ev")} value={formatPercent(bet.ev)} tone="blue" /><label className="rounded-2xl border border-[var(--sc-border)] bg-black/20 p-4 text-xs font-bold uppercase tracking-[0.12em] text-[var(--sc-muted)]">{tr({ fi: "Päätöskerroin", en: "Closing odds", es: "Cuota de cierre" })}<input disabled={itemBusy} value={closingDrafts[bet.id] ?? ""} onChange={(event) => changeClosingOdds(bet.id, event.target.value)} placeholder="1.95" inputMode="decimal" className="mt-3 w-full rounded-xl border border-[var(--sc-border)] bg-[var(--sc-surface)] px-3 py-2 text-base font-black normal-case tracking-normal text-[var(--sc-text)]" /><button type="button" onClick={() => void settleBet(bet.id, bet.result, true)} disabled={itemBusy || String(closingDrafts[bet.id] ?? "") === String(bet.closingOdds ?? "")} className="mt-3 text-xs font-bold normal-case tracking-normal text-[var(--sc-brand)] underline disabled:opacity-40">{tr({ fi: "Tallenna kerroin", en: "Save closing odds", es: "Guardar cuota de cierre" })}</button></label><MetricTile label="CLV" value={clv === null ? "—" : formatPercent(clv)} tone={clv === null ? "default" : clv >= 0 ? "green" : "red"} /><MetricTile label={t("home.paperResult")} value={money(profit)} tone={profit >= 0 ? "green" : "red"} /></div>
                {bet.riskWarnings?.length ? <details className="mt-5 rounded-2xl border border-amber-300/20 bg-amber-300/10 p-4"><summary className="cursor-pointer font-bold text-amber-200">{tr({ fi: "Näytä riskivaroitukset", en: "Show risk warnings", es: "Mostrar avisos de riesgo" })}</summary><ul className="mt-3 space-y-1 text-sm text-[var(--sc-text-secondary)]">{bet.riskWarnings.map((warning) => <li key={warning}>• {warning}</li>)}</ul></details> : null}
                <MatchStoryCard bet={bet} tr={tr} locale={locale} />
                <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap">{bet.result === "pending" ? <><button type="button" disabled={itemBusy} onClick={() => void settleBet(bet.id, "win")} className="sc-button-primary disabled:opacity-50">{tr({ fi: "Merkitse voitoksi", en: "Mark as win", es: "Marcar victoria" })}</button><button type="button" disabled={itemBusy} onClick={() => void settleBet(bet.id, "loss")} className="rounded-xl border border-rose-300/30 bg-rose-300/10 px-4 py-3 text-sm font-black text-[var(--sc-text)] disabled:opacity-50">{tr({ fi: "Merkitse tappioksi", en: "Mark as loss", es: "Marcar derrota" })}</button><button type="button" disabled={itemBusy} onClick={() => void settleBet(bet.id, "push")} className="rounded-xl border border-amber-300/30 bg-amber-300/10 px-4 py-3 text-sm font-black text-[var(--sc-text)] disabled:opacity-50">{tr({ fi: "Merkitse palautukseksi", en: "Mark as push", es: "Marcar nulo" })}</button></> : <button type="button" disabled={itemBusy} onClick={() => void settleBet(bet.id, "pending")} className="sc-button-secondary disabled:opacity-50">{tr({ fi: "Avaa uudelleen", en: "Reopen", es: "Reabrir" })}</button>}<button type="button" disabled={itemBusy} onClick={() => void removeBet(bet.id)} className="sc-button-ghost disabled:opacity-50">{itemBusy ? tr({ fi: "Odota…", en: "Please wait…", es: "Espera…" }) : t("common.delete")}</button></div>
              </article>;
            })}
          </div>
        )}
