...
export default function VerifiedLiveMonitorClient() {
  ...
  return <div className="space-y-7">
    ...
    {health && <div className="rounded-2xl border border-[var(--sc-border)] bg-[var(--sc-surface-soft)] p-4 text-sm text-[var(--sc-text-secondary)]"><div>Pipeline: <strong className="text-[var(--sc-text)]">{health.status}</strong> · snapshots 24h {health.last24Hours?.snapshots ?? 0} · alerts 24h {health.last24Hours?.alerts ?? 0} · provider {health.provider?.contractReady ? "contract ready" : "blocked"}</div>{health.provider?.failedGates?.length ? <div className="mt-3 flex flex-wrap gap-2">{health.provider.failedGates.map((gate) => <span key={gate} className="rounded-full border border-amber-400/25 bg-amber-400/10 px-2.5 py-1 text-xs font-bold text-amber-100">{gate}</span>)}</div> : null}<Link href="/data-readiness" className="mt-3 inline-flex font-black text-[var(--sc-brand)]">Open full data readiness →</Link></div>}
