  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const bankrollPlan = calculateBankrollPlan({
          bankroll,
          picks: data.data || []
        });
    ...
    <div className="space-y-6">
      <section className="rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900 to-slate-950 p-6 shadow-2xl">
        ...
        <p className="mt-3 text-slate-300">
          Hallitsee panoskokoa, kokonaisriskiä ja agentin BET-kohteiden
          allokaatiota.
        </p>
      ...
      <section className="grid gap-4 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
          ...
          <div className="mt-2 text-3xl font-black text-yellow-300">
            {formatPercent(plan.allocated / plan.bankroll)}
          </div>
        </div>
      </section>
      ...
      <Panel title="Recommended Positions" subtitle="BET-only allocations">
        <div className="space-y-3">
          {plan.recommendations.length === 0 && (
            <div className="rounded-xl bg-white/[0.04] p-4 text-sm text-slate-400">
              Ei BET-tason kohteita juuri nyt.
            </div>
          )}
              ...
              <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                <div>
                  ...
                  <div className="mt-1 text-sm text-purple-300">
                    Score: {formatPercent(bet.finalScore)}
                  </div>
                </div>
