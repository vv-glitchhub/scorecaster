...
import { GET as runCollector } from "../collector/route";
...
async function invokeTask(request, task) {
  const cronSecret = process.env.CRON_SECRET;
  if (!cronSecret) {
    return response({ ok: false, error: "Production worker secret is not configured", paperOnly: true }, 503);
  }
...
export async function POST(request) {
  ...
  if (!TASKS.has(task)) return response({ ok: false, error: "Unsupported scheduler task", paperOnly: true }, 400);

  try {
    return await invokeTask(request, task);
  } catch (error) {
