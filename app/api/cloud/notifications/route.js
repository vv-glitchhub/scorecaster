  ...
  if (error) {
    if (isMissingTable(error)) {
      return {
        ...
        devices: [],
        warning: "Notification registry migration is not active"
      };
    }
...
async function requireMutation(request, requestId, bucket, limit = 30) {
  if (!mutationOriginAllowed(request)) {
    return { response: jsonResponse({ ok: false, error: "Invalid request origin" }, 403, requestId) };
  }

  const auth = await getAuthenticatedContext(request);
  if (!auth.ok) {
    return { response: jsonResponse({ ok: false, error: auth.error }, auth.status, requestId) };
  }
...
export async function GET(request) {
  ...
  const limited = await enforceRateLimit(auth, requestId, {
    bucket: "notification_registry_read",
    limit: 60,
    windowSeconds: 60
  });
  ...
  if (state.error) {
    return jsonResponse({ ok: false, error: publicError(state.error, "Notification settings could not be loaded") }, 500, requestId);
  }
...
export async function PUT(request) {
  ...
  if (error) {
    ...
    return jsonResponse({ ok: false, error: publicError(error, "Notification preferences could not be saved") }, status, requestId);
  }

  const state = await loadState(auth);
  return jsonResponse({ ok: true, deliveryActive: false, ...state }, 200, requestId);
}
...
export async function POST(request) {
  ...
  if (!TOKEN_PATTERN.test(token) || !["ios", "android"].includes(platform)) {
    return jsonResponse({ ok: false, error: "A valid Expo push token and supported platform are required" }, 400, requestId);
  }

  const { data: deviceId, error: claimError } = await auth.supabase.rpc("claim_notification_device", {
  ...
  if (claimError) {
    const status = isMissingTable(claimError) ? 503 : 500;
    return jsonResponse({ ok: false, error: publicError(claimError, "Notification device could not be registered") }, status, requestId);
  }
  ...
  const state = await loadState(auth);
  return jsonResponse({ ok: true, deviceId, deliveryActive: false, ...state }, 200, requestId);
}
...
export async function DELETE(request) {
  ...
  if (error) {
    const status = isMissingTable(error) ? 503 : 500;
    return jsonResponse({ ok: false, error: publicError(error, "Notification device could not be removed") }, status, requestId);
  }
