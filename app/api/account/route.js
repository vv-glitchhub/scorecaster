...
export async function GET(request) {
  ...
  return jsonResponse({
    ...
    deletes: ["account", "profile", "paper bets", "paper bankroll settings", "verified watchlist", "Alert Inbox history and settings", "tracking history"],
...
export async function DELETE(request) {
  ...
  if (phrase !== CONFIRMATION || !authenticatedEmail || email !== authenticatedEmail) {
    return jsonResponse({ ok: false, error: "The confirmation phrase or account email did not match" }, 400, requestId);
  }

  for (const table of USER_TABLES) {
    const { error } = await admin.from(table).delete().eq("user_id", auth.user.id);
    if (error && !isMissingTable(error)) {
      return jsonResponse({ ok: false, error: publicError(error, "Account data deletion failed") }, 500, requestId);
    }
  }

  const { error: profileError } = await admin.from("profiles").delete().eq("id", auth.user.id);
  if (profileError && !isMissingTable(profileError)) {
    return jsonResponse({ ok: false, error: publicError(profileError, "Profile deletion failed") }, 500, requestId);
  }

  const { error: deleteUserError } = await admin.auth.admin.deleteUser(auth.user.id);
  if (deleteUserError) {
    return jsonResponse({ ok: false, error: publicError(deleteUserError, "Account deletion failed") }, 500, requestId);
  }
