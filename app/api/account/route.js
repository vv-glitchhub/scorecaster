  ...
  return jsonResponse(
    {
      ...
      deletes: ["account", "profile", "paper bets", "paper bankroll settings", "verified watchlist", "alert inbox", "notification preferences", "notification device registrations", "tracking history"],
...
export async function DELETE(request) {
  ...
  if (!admin) {
    return jsonResponse(
      { ok: false, error: "Account deletion is not configured on the server" },
      503,
      requestId
    );
  }
  ...
  if (phrase !== CONFIRMATION || !authenticatedEmail || email !== authenticatedEmail) {
    return jsonResponse(
      { ok: false, error: "The confirmation phrase or account email did not match" },
      400,
      requestId
    );
  }
  ...
  for (const table of USER_TABLES) {
    ...
    if (error && !isMissingTable(error)) {
      return jsonResponse(
        { ok: false, error: publicError(error, "Account data deletion failed") },
        500,
        requestId
      );
    }
  ...
  if (profileError && !isMissingTable(profileError)) {
    return jsonResponse(
      { ok: false, error: publicError(profileError, "Profile deletion failed") },
      500,
      requestId
    );
  }
  ...
  if (deleteUserError) {
    return jsonResponse(
      { ok: false, error: publicError(deleteUserError, "Account deletion failed") },
      500,
      requestId
    );
  }
