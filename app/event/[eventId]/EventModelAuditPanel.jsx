  ...
  return (
    <section className="sc-surface rounded-[1.65rem] p-5 sm:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-xl border border-[var(--sc-border)] bg-[var(--sc-surface-soft)] p-4"><div className="text-[10px] font-black uppercase tracking-[0.14em] text-[var(--sc-faint)]">{tr({ fi: "Research gate", en: "Research gate", es: "Research gate" })}</div><div className="mt-2"><AuditBadge ok={riskGate?.blocked !== true}>{compact(riskGate?.decision, "NO_BET")}</AuditBadge></div><div className="mt-2 text-xs text-[var(--sc-muted)]">{riskGate?.blocked === true ? tr({ fi: "mallikerros ei vielä läpäise tutkimusrajaa", en: "model layer does not yet pass the research gate", es: "la capa de modelos aún no supera el límite" }) : tr({ fi: "mallikerros valmis tarkasteluun", en: "model layer ready for review", es: "capa lista para revisión" })}</div></div>
      </div>
