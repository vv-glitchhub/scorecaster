  ...
  return (
    <article className="sc-surface-soft rounded-2xl p-4" aria-label={`${provider?.provider || "provider"} diagnostics`}>
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <span className="rounded-full border border-[var(--sc-border)] px-2.5 py-1 text-xs font-black">
          {percent(provider?.usableRate)}
        </span>
      </div>
  ...
  return (
    <section className="sc-surface rounded-3xl p-5 sm:p-6" aria-labelledby="pricing-provider-diagnostics-title">
      ...
      <dl className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label={tr({ fi: "Pricing availability", en: "Pricing availability", es: "Disponibilidad de precios" })} value={percent(readiness.averagePricingAvailability)} detail={`${readiness.pricingProviderCount ?? 0} pricing providers`} />
        ...
        <Metric label={tr({ fi: "Ei-eligible odds-havainnot", en: "Non-eligible odds observations", es: "Observaciones de cuotas no elegibles" })} value={readiness.nonEligibleOddsObservationCount ?? data?.summary?.nonEligibleOddsObservations ?? 0} detail={tr({ fi: "unsupported / not configured", en: "unsupported / not configured", es: "no compatible / no configurado" })} />
      ...
      {!hasEvidence ? (
        <div className="mt-5 rounded-2xl border border-[var(--sc-border)] p-4 text-sm text-[var(--sc-muted)]">
          {tr({
            fi: "Uutta provider-diagnostiikkaa ei ole vielä tallentunut. Näkymä täyttyy seuraavien scheduled worker -havaintojen myötä.",
            en: "No new provider diagnostics have been retained yet. This panel will fill after fresh scheduled worker observations.",
            es: "Aún no hay nuevos diagnósticos retenidos. El panel se completará tras nuevas observaciones programadas."
          })}
        </div>
      ) : (
        <>
          ...
          <div className="mt-6 overflow-x-auto">
            <table className="min-w-[1040px] w-full text-left text-sm">
              ...
              <thead className="text-[10px] font-black uppercase tracking-[0.12em] text-[var(--sc-muted)]">
                <tr>
                  <th className="p-3">Provider / league</th>
                  <th className="p-3">Live coverage</th>
                  <th className="p-3">Eligible</th>
                  <th className="p-3">Modes</th>
                  <th className="p-3">Upstream</th>
                  <th className="p-3">Match gate</th>
                </tr>
              </thead>
              <tbody>
                {leagues.map((row, index) => {
                  ...
                  return (
                    <tr key={`${row.provider}:${row.sport}:${row.league}:${index}`} className="border-t border-[var(--sc-border)] align-top">
                      <td className="p-3">
                        <div className="font-black text-[var(--sc-text)]">{row.provider}</div>
                        <div className="mt-1 text-xs text-[var(--sc-muted)]">{row.league} · {row.sport}</div>
                      </td>
