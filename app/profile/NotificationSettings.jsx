...
export default function NotificationSettings() {
  ...
  async function load() {
    ...
    try {
      ...
      if (!response.ok) throw Object.assign(new Error("Notification settings unavailable"), { status: response.status });
      setState(payload);
    } catch (loadError) {
      setError(requestErrorText(loadError, tr));
    }
  }
  ...
  async function toggle(key) {
    ...
    try {
      ...
      const payload = await response.json();
      if (!response.ok) throw Object.assign(new Error("Preference update failed"), { status: response.status });
      setState(payload);
    } catch (updateError) {
      setError(requestErrorText(updateError, tr));
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <section className="rounded-[2rem] border border-white/10 bg-white/[0.04] p-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <button type="button" onClick={() => void load()} disabled={busy || (state === null && !error)} className="rounded-xl border border-white/10 bg-white/5 px-4 py-2 text-sm font-black text-white disabled:opacity-50">{tr({ fi: "Päivitä", en: "Refresh", es: "Actualizar" })}</button>
      </div>
      {error && <div className="mt-4 rounded-xl border border-red-400/20 bg-red-400/10 p-3 text-sm text-red-100">{error}</div>}
      {state === null && !error ? <div className="mt-5 rounded-xl border border-white/10 bg-slate-950/60 p-4 text-sm text-slate-400" role="status" aria-live="polite">{tr({ fi: "Ladataan ilmoitusasetuksia…", en: "Loading notification settings…", es: "Cargando la configuración de notificaciones…" })}</div> : state && <>
...
function Metric({ label, value }) { return <div className="rounded-xl border border-white/10 bg-slate-950/60 p-4"><div className="text-xs text-slate-500">{label}</div><div className="mt-1 font-black text-white">{value}</div></div>; }
