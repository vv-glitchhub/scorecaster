    ...
    <article className="overflow-hidden rounded-[2rem] border border-emerald-400/35 bg-emerald-500/10 shadow-2xl">
      ...
      <div className="p-5 sm:p-6">
        ...
        <div className="mt-5 rounded-2xl border border-emerald-400/25 bg-[var(--sc-surface)]/70 p-5">
          ...
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm font-bold text-[var(--sc-text-secondary)]"><span>{marketLabel(item.marketKey, tr)}</span><span>@ {number(item.odds)}</span>{item.bookmaker ? <span>{item.bookmaker}</span> : null}</div>
  ...
  return (
    <div className="space-y-7">
      <section className="overflow-hidden rounded-[2rem] border border-[var(--sc-brand-border)] bg-[var(--sc-brand-soft)] p-6 shadow-2xl sm:p-8">
        <div className="flex flex-wrap items-end justify-between gap-5"><div><div className="text-xs font-black uppercase tracking-[0.2em] text-[var(--sc-brand)]">{tr({ fi: "Scorecaster tänään", en: "Scorecaster today", es: "Scorecaster hoy" })}</div><h1 className="mt-2 text-4xl font-black tracking-[-0.05em] text-[var(--sc-text)] sm:text-5xl">{tr({ fi: "Mitä pelata nyt?", en: "What to play now?", es: "¿Qué jugar ahora?" })}</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--sc-muted)] sm:text-base">{tr({ fi: "PLAY-kohteet näytetään aina ensin kaikista aktiivisista tuetuista lajeista. Jos yhtään kohdetta ei läpäise portteja, Scorecaster sanoo sen suoraan eikä muuta CAUTION-kohdetta pelisuositukseksi.", en: "PLAY picks are always shown first across active supported sports. If none pass the gates, Scorecaster says so instead of presenting CAUTION as a bet recommendation.", es: "Los PLAY aparecen primero entre los deportes activos compatibles. Si ninguno pasa los filtros, Scorecaster lo indica sin convertir CAUTION en recomendación." })}</p></div><div className="rounded-2xl border border-[var(--sc-border)] bg-[var(--sc-surface)]/70 px-5 py-4 text-right"><div className="text-3xl font-black text-[var(--sc-text)]">{loading ? "…" : plays.length}</div><div className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--sc-muted)]">PLAY</div><button type="button" onClick={() => load()} className="mt-2 text-xs font-black text-[var(--sc-brand)]">{tr({ fi: "Päivitä", en: "Refresh", es: "Actualizar" })}</button></div></div>
        <div className="mt-5 flex flex-wrap gap-2 text-xs text-[var(--sc-muted)]"><span>{tr({ fi: "Päivitetty", en: "Updated", es: "Actualizado" })}: {updated}</span><span>·</span><span>{tr({ fi: "Aktiivisia liigahakuja", en: "Active league searches", es: "Ligas activas consultadas" })}: {leagues.length || "–"}</span><span>·</span><span>{tr({ fi: "vain paperianalyysi", en: "paper analysis only", es: "solo análisis simulado" })}</span></div>
      </section>

      {error ? <div className="rounded-2xl border border-red-400/30 bg-red-500/10 p-5 text-sm text-red-100">{error}</div> : null}
      {loading ? <div className="grid gap-4 lg:grid-cols-2">{[1, 2].map((item) => <div key={item} className="h-96 animate-pulse rounded-[2rem] border border-[var(--sc-border)] bg-[var(--sc-surface)]" />)}</div> : plays.length ? <section><div className="mb-4 text-xs font-black uppercase tracking-[0.18em] text-emerald-300">{tr({ fi: "Pelaa nämä ensin", en: "Play these first", es: "Juega estos primero" })}</div><div className="grid gap-5 lg:grid-cols-2">{plays.map((item) => <PlayCard key={`${item.eventId}-${item.selection}-${item.marketKey}`} item={item} tr={tr} onWatch={addToWatchlist} watchState={watchState} />)}</div></section> : <section className="rounded-[2rem] border border-amber-400/30 bg-amber-500/10 p-6 sm:p-8"><div className="text-xs font-black uppercase tracking-[0.18em] text-amber-200">WAIT</div><h2 className="mt-2 text-3xl font-black tracking-[-0.04em] text-[var(--sc-text)]">{tr({ fi: "Ei varmennettua PLAY-kohdetta juuri nyt", en: "No verified PLAY pick right now", es: "No hay PLAY verificado ahora" })}</h2><p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--sc-muted)]">{tr({ fi: "Älä pelaa CAUTION-kohteita pelkän markkinaedgen perusteella. Alla näkyvät vain lähimpänä PLAYta olevat seurattavat ehdokkaat ja täsmälleen se ehto, joka vielä puuttuu.", en: "Do not play CAUTION picks from market edge alone. Below are only the candidates closest to PLAY and the exact gate still missing.", es: "No juegues CAUTION solo por ventaja de mercado. Abajo se muestran los candidatos más cercanos a PLAY y el filtro que falta." })}</p></section>}

      {!loading && nearPlay.length ? <section><div className="mb-4 flex items-end justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-[0.18em] text-amber-200">{tr({ fi: "Lähimpänä PLAYta", en: "Closest to PLAY", es: "Más cerca de PLAY" })}</div><h2 className="mt-1 text-2xl font-black text-[var(--sc-text)]">{tr({ fi: "Seuraa – älä pelaa vielä", en: "Watch – do not play yet", es: "Sigue – todavía no juegues" })}</h2></div><Link href="/feed" className="text-sm font-black text-[var(--sc-brand)]">{tr({ fi: "Kaikki analyysit", en: "All analysis", es: "Todos los análisis" })}</Link></div><div className="space-y-3">{nearPlay.map((item) => <article key={`${item.eventId}-${item.selection}-${item.marketKey}`} className="rounded-3xl border border-amber-400/20 bg-[var(--sc-surface)] p-5"><div className="flex items-start justify-between gap-3"><div><div className="text-xs font-bold text-[var(--sc-muted)]">{item.sportTitle || item.league || item.sportKey}</div><div className="mt-1 text-xl font-black text-[var(--sc-text)]">{item.match}</div></div><div className="rounded-full border border-amber-400/30 bg-amber-500/10 px-3 py-1 text-[10px] font-black text-amber-100">WAIT</div></div><div className="mt-4 grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto_auto_auto] sm:items-center"><div><div className="text-[10px] font-bold uppercase text-[var(--sc-faint)]">{tr({ fi: "Seurattava ehdokas – ei pelisuositus", en: "Candidate to watch – not a bet recommendation", es: "Candidato a seguir – no recomendación" })}</div><div className="mt-1 font-black text-[var(--sc-text)]">{item.selection || "–"} @ {number(item.odds)}</div></div><div><div className="text-[10px] text-[var(--sc-faint)]">Edge</div><div className="font-black text-[var(--sc-text)]">{percent(item.edge)}</div></div><div><div className="text-[10px] text-[var(--sc-faint)]">EV</div><div className="font-black text-[var(--sc-text)]">{percent(item.ev)}</div></div><Link href={recommendationHref(item)} className="rounded-xl border border-[var(--sc-border)] px-4 py-3 text-center text-sm font-black text-[var(--sc-text)]">{tr({ fi: "Avaa", en: "Open", es: "Abrir" })}</Link></div><div className="mt-4"><ModelStrip item={item} tr={tr} /></div><div className="mt-3 rounded-xl bg-amber-500/10 px-4 py-3 text-sm text-amber-100"><span className="font-black">{tr({ fi: "Puuttuu:", en: "Still missing:", es: "Falta:" })}</span> {gateText(item, tr)}</div></article>)}</div></section> : null}
