...
export default function AppShell({ children }) {
  ...
  const alertInboxItem = useMemo(() => ({
    ...
    description: tr({ fi: "Lokalisoitu hälytyshistoria ja omat suodatusasetukset.", en: "Localized alert history and personal filtering settings.", es: "Historial localizado y configuración personal de filtros." })
  }), [tr]);
  ...
  ], [alertInboxItem, t, watchlistItem, formRestLabItem]);
      ...
      <header className="sticky top-0 z-50 border-b border-white/10 bg-slate-950/95 backdrop-blur-xl">
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex items-center justify-between gap-3 py-3 md:py-4">
            ...
            <div className="hidden items-center gap-2 md:flex"><div className="rounded-xl border border-yellow-400/20 bg-yellow-400/10 px-3 py-2 text-xs font-black text-yellow-200">{t("mode.paper")}</div><LanguageSwitcher compact /><Link href="/alerts" className="rounded-xl border border-fuchsia-400/20 bg-fuchsia-400/10 px-3 py-2 text-sm font-bold text-fuchsia-200">{tr({ fi: "Hälytykset", en: "Alerts", es: "Alertas" })}</Link><Link href="/help" className="rounded-xl border border-white/10 bg-white/[0.04] px-3 py-2 text-sm font-bold text-slate-300 hover:bg-white/[0.08]">{t("nav.help")}</Link></div>
