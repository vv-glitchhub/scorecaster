        ...
        <div className="mx-auto max-w-7xl px-4">
          <div className="flex items-center justify-between gap-3 py-3 md:py-4">
            ...
            <div className="flex items-center gap-2 lg:hidden">
              <LanguageSwitcher compact />
              <button type="button" onClick={() => setMenuOpen((value) => !value)} className="rounded-xl border border-white/10 bg-white/[0.05] px-4 py-3 text-sm font-black text-white" aria-expanded={menuOpen} aria-controls="mobile-navigation">{menuOpen ? t("nav.close") : t("nav.menu")}</button>
            </div>
          </div>

          <div className="hidden items-center gap-2 pb-4 lg:flex">
            <nav className="flex flex-1 gap-2" aria-label={t("nav.mainAria")}>
              {primaryItems.map((item) => <Link key={item.href} href={item.href} className={`whitespace-nowrap rounded-xl px-4 py-2.5 text-sm font-bold transition-all ${isActive(item) ? "bg-emerald-400 text-slate-950" : "border border-white/10 bg-white/[0.04] text-slate-300 hover:bg-white/[0.08]"}`}>{item.label}</Link>)}
            </nav>

            <div className="relative">
              <button type="button" onClick={() => setMoreOpen((value) => !value)} className="rounded-xl border border-white/10 bg-white/[0.04] px-4 py-2.5 text-sm font-bold text-slate-300 hover:bg-white/[0.08]" aria-expanded={moreOpen}>{t("nav.more")} ▾</button>
              {moreOpen && <div className="absolute right-0 top-12 z-50 w-[680px] max-w-[90vw] rounded-2xl border border-white/10 bg-slate-900 p-4 shadow-2xl"><div className="grid gap-4 md:grid-cols-3">{secondaryGroups.map((group) => <section key={group.title}><h2 className="mb-2 text-xs font-black uppercase tracking-[0.18em] text-slate-500">{group.title}</h2><div className="space-y-2">{group.items.map((item) => <Link key={item.href} href={item.href} className="block rounded-xl border border-white/10 bg-white/[0.03] p-3 hover:bg-white/[0.08]"><div className="text-sm font-black text-white">{item.label}</div><div className="mt-1 text-xs leading-5 text-slate-400">{item.description}</div></Link>)}</div></section>)}</div></div>}
            </div>
          </div>
