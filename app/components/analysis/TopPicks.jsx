    ...
    <section className="space-y-4">
      ...
      {picks.map((bet, index) => (
        <div
          ...
          className={`rounded-[28px] border p-6 shadow-lg ${bet.isBet ? "border-green-700 bg-[#08183E]" : "border-orange-700 bg-[#08183E]"}`}
        >
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <div className="text-sm font-semibold uppercase tracking-wide text-slate-300">#{index + 1} Suositus</div>
              <div className="mt-1 text-2xl font-extrabold text-white">{bet.outcomeName}</div>
              <div className="mt-1 text-sm text-slate-300">{formatOdds(bet.odds)} • {bet.bookmaker}</div>
            </div>
