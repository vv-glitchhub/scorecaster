...
test("notification migration forces RLS and computes token hash in PostgreSQL", async () => {
  ...
  assert.doesNotMatch(sql, /grant insert on public\.notification_devices to authenticated/i);
});
...
test("notification API checks origin, auth and quota before mutations", async () => {
  ...
  assert.ok(originIndex > helperStart);
...
test("push preference cannot be enabled without a successful device claim", async () => {
  ...
  assert.ok(putStart >= 0 && postStart > putStart);
  assert.ok(claimIndex > postStart);
  assert.ok(pushEnableIndex > claimIndex);
...
test("Watchlist applies preferences before synchronizing Alert Inbox", async () => {
  ...
  assert.ok(filterIndex > buildIndex);
...
test("account deletion removes notification devices before preferences and account data", async () => {
  ...
  assert.ok(preferenceIndex > deviceIndex);
  assert.ok(inboxIndex > preferenceIndex);
...
test("native registration is explicit, permission-gated and requires EAS project ID", async () => {
  ...
  assert.doesNotMatch(settings, /registerNotificationDevice\(\).*useEffect/s);
});
