...
test("unified data service forwards only bounded SportsGameOdds confidence, rejection and upstream evidence", async () => {
  ...
  assert.match(service, /safeSportsGameOddsUpstreamEvidence\(\{/);
  ...
  assert.match(providerSection, /upstream/);
  assert.doesNotMatch(providerSection, /candidateCount|providerHomeTeam|providerAwayTeam|eventId\s*:|rawProvider|apiKey|errorText|errorMessage/i);
});
