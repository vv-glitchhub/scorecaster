...
const report = {
  version: "production-activation-v2-autonomous-v12",
  action,
  ...
  autonomousV12SchemaVerified: false,
  probes: [],
  health: null,
  error: null
};
...
async function verifySchema() {
  ...
  runPsql(["--file=scripts/verify-autonomous-v12-schema.sql"], "Autonomous Intelligence V12 schema verification");
  report.autonomousV12SchemaVerified = true;
}
...
async function migrate() {
  ...
  assert(migrations.at(-3) === "supabase/scorecaster_autonomous_intelligence_v12.sql", "Autonomous Intelligence V12 migration must run before the final worker migrations");
...
async function probeWorkers() {
  ...
  for (const workerPath of workers) {
    ...
    if (workerPath === "/api/internal/autonomous-agent") {
      assert(/autonomous-(intelligence-v12|paper-agent-v1)/.test(version), "Autonomous worker did not report a reviewed version");
    }
  }
}
