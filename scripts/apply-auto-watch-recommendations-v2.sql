...
-- Production migration registry name: scorecaster_auto_watch_recommendations_v2
-- Expands the existing V1 preference registry with user-controlled paper-only filters.
...
begin
  ...
  if p_top_n is null or p_top_n < 1 or p_top_n > 10 then raise exception 'top_n must be between 1 and 10'; end if;
  if p_alert_move_percent is null or p_alert_move_percent < 0.005 or p_alert_move_percent > 0.5 then raise exception 'alert_move_percent is outside the allowed range'; end if;
  if p_alert_before_minutes is null or p_alert_before_minutes < 15 or p_alert_before_minutes > 10080 then raise exception 'alert_before_minutes is outside the allowed range'; end if;
  if p_selection_mode not in ('play-only', 'play-and-caution') then raise exception 'selection_mode is unsupported'; end if;
  if p_min_score is null or p_min_score < 0 or p_min_score > 100 then raise exception 'min_score is outside the allowed range'; end if;
  if p_min_edge is null or p_min_edge < 0 or p_min_edge > 0.20 then raise exception 'min_edge is outside the allowed range'; end if;
  if p_min_ev is null or p_min_ev < 0 or p_min_ev > 1.00 then raise exception 'min_ev is outside the allowed range'; end if;
  ...
  ) then raise exception 'sport_keys contains an invalid filter'; end if;
