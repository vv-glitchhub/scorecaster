...
test("migration rollout follows the reviewed V13 manifest and uses fail-fast transactions", async () => {
  const runner = await source("scripts/production-activation.mjs");
  ...
  assert.match(runner, /verify-autonomous-v13-schema\.sql/);
  assert.match(runner, /autonomousV13Verified/);
  assert.match(runner, /sha256/);
});

test("schema verifiers check base and V13 RLS, grants, cooldowns and audit storage", async () => {
  const sql = await source("scripts/verify-production-schema.sql");
  const v13 = await source("scripts/verify-autonomous-v13-schema.sql");
  ...
  assert.match(v13, /autonomous_agent_decision_audit/);
  assert.match(v13, /autonomous_agent_daily_briefs/);
  assert.match(v13, /complete_autonomous_agent_user_v2/);
  assert.match(v13, /paused_until/);
  assert.match(v13, /service_role cannot write Autonomous V13 decision audit/);
  assert.match(v13, /Authenticated users must not write Autonomous V13 decision audit/);
