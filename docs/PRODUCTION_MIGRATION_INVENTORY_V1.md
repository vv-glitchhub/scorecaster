...
The canonical ordered rollout now contains 28 migrations. The inventory includes every reviewed `supabase/scorecaster_*.sql` migration, including the independently verified Agent signing migration, Autonomous Risk Profile V1 and the five production-verified Shadow Candidate migrations. During the original inventory work package, two migrations were found in `supabase/` but missing from the release manifest:
