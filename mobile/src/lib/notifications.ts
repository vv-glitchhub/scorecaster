...
export async function registerNotificationDevice() {
  if (Platform.OS !== "ios" && Platform.OS !== "android") {
    throw new Error("Push notifications require the native iOS or Android application.");
  }
  ...
  if (!easProjectId) {
    throw new Error("The EAS project ID is not configured. Device registration remains disabled.");
  }
  ...
  await SecureStore.setItemAsync(DEVICE_ID_KEY, response.deviceId, {
