...
export default function EventDetailScreen({ pick, onBack }: Props) {
  ...
  async function load() {
    ...
    try {
      ...
      setSelectedName((current) => current || eventResponse.detail.selectedSelection || eventResponse.detail.selections[0]?.selection || "");
    } catch (error) {
      setDetail(null);
