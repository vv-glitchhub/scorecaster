  ...
  return (
    <ScrollView contentContainerStyle={styles.screen}>
      ...
      {data ? <>
        ...
        <Card><Text style={styles.cardTitle}>{tr({ fi: "Automaattiset hälytykset", en: "Automatic alerts", es: "Alertas automáticas" })}</Text>{activeAlerts.length === 0 ? <Text style={styles.muted}>{tr({ fi: "Aktiivisia all-SKIP-, stale-data- tai provider-hälytyksiä ei ole.", en: "No active all-SKIP, stale-data or provider alerts.", es: "No hay alertas activas." })}</Text> : activeAlerts.map((item) => <View key={item.fingerprint} style={[local.alert, item.severity === "high" && local.alertHigh]}><Text style={local.alertTitle}>{item.title}</Text><Text style={styles.muted}>{item.message}</Text></View>)}</Card>
