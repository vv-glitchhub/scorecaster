...
export default function PicksScreen() {
  ...
  async function savePick(pick: Pick, index: number) {
    ...
    if (!match || !selection || odds <= 1) {
      Alert.alert(tr({ fi: "Kohde puutteellinen", en: "Incomplete pick", es: "Pronóstico incompleto" }), tr({ fi: "Kohteen tietoja ei voida tallentaa turvallisesti.", en: "The pick cannot be stored safely because data is missing.", es: "El pronóstico no se puede guardar de forma segura porque faltan datos." }));
      return;
    }
    if (!limits.allowed) {
      Alert.alert(tr({ fi: "Oma paperiraja estää tallennuksen", en: "Your paper limit blocks saving", es: "Tu límite simulado impide guardar" }), tr({ fi: `Vaadittu edge on ${percent(limits.minEdge)} ja confidence ${percent(limits.minConfidence)}.`, en: `Required edge is ${percent(limits.minEdge)} and confidence ${percent(limits.minConfidence)}.`, es: `La ventaja requerida es ${percent(limits.minEdge)} y la confianza ${percent(limits.minConfidence)}.` }));
      return;
    }
    if (stake === null || stake <= 0 || stake > maximum + 0.001) {
      Alert.alert(tr({ fi: "Tarkista paperipanos", en: "Check the paper stake", es: "Revisa el importe simulado" }), tr({ fi: `Anna panos väliltä 0,01–${money(maximum)}.`, en: `Enter a stake between 0.01 and ${money(maximum)}.`, es: `Introduce un importe entre 0,01 y ${money(maximum)}.` }));
      return;
    }
  ...
  const decisionItems: { key: DecisionFilter; label: string }[] = [
    { key: "all", label: tr({ fi: "Kaikki päätökset", en: "All decisions", es: "Todas las decisiones" }) },
    { key: "PLAY", label: "PLAY" },
    { key: "CAUTION", label: "CAUTION" }
  ];
  const sortItems: { key: SortMode; label: string }[] = [
    { key: "rank", label: tr({ fi: "Paras ensin", en: "Best first", es: "Mejores primero" }) },
    { key: "edge", label: "Edge" },
    { key: "confidence", label: "Confidence" },
    { key: "time", label: tr({ fi: "Alkamisaika", en: "Kickoff", es: "Hora de inicio" }) }
  ];

  return (
    <ScrollView contentContainerStyle={styles.screen} keyboardShouldPersistTaps="handled">
      <View style={styles.rowBetween}><View style={{ flex: 1 }}><Text style={styles.title}>{tr({ fi: "Päivän kohteet", en: "Today's picks", es: "Pronósticos de hoy" })}</Text><Text style={styles.subtitle}>{tr({ fi: "Paras hinta verrataan marginaalista puhdistettuun markkinakonsensukseen.", en: "The best price is compared with no-vig market consensus.", es: "La mejor cuota se compara con el consenso de mercado sin margen." })}</Text></View><ActionButton label={tr({ fi: "Päivitä", en: "Refresh", es: "Actualizar" })} onPress={() => load()} tone="secondary" compact disabled={loading} /></View>

      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filterRow}>{FILTERS.map((item) => { const active = item.key === filter.key; return <Pressable accessibilityLabel={`${tr({ fi: "Valitse liiga", en: "Choose league", es: "Elegir liga" })} ${filterLabel(item)}`} accessibilityRole="button" accessibilityState={{ selected: active }} key={item.key} onPress={() => setFilter(item)} style={[styles.filterChip, active && styles.filterChipActive]}><Text style={[styles.filterText, active && styles.filterTextActive]}>{filterLabel(item)}</Text></Pressable>; })}</ScrollView>
      ...
      {visiblePicks.map((pick, index) => {
        ...
        return <Card key={`${id}-${index}`}>
          ...
          <Text style={styles.cardTitle}>{match}</Text><Text style={styles.value}>{pick.selection || pick.label || tr({ fi: "Valinta", en: "Selection", es: "Selección" })} · {Number(pick.odds || 0).toFixed(2)}</Text><Text style={styles.muted}>{formatKickoff(pick.commenceTime)} · {pick.bookmaker || tr({ fi: "Paras saatavilla oleva hinta", en: "Best available price", es: "Mejor cuota disponible" })}</Text>
          <View style={styles.divider} /><Text style={styles.muted}>{tr({ fi: "Konsensus", en: "Consensus", es: "Consenso" })} {percent(consensusProbability)} · {tr({ fi: "markkina", en: "market", es: "mercado" })} {percent(marketProbability)} · edge {percent(pick.edge)} · EV {percent(pick.ev)}</Text><Text style={styles.muted}>{tr({ fi: "Reilu kerroin", en: "Fair odds", es: "Cuota justa" })} {pick.fairOdds ? Number(pick.fairOdds).toFixed(2) : "–"} · confidence {percent(pick.confidence)} · trust {Number(pick.trustScore || 0).toFixed(0)}/100</Text><Text style={styles.muted}>{Number(pick.bookmakerCount || pick.dataQuality?.bookmakerCount || 0)} {tr({ fi: "lähdettä", en: "sources", es: "fuentes" })} · {dataFreshness(pick)}</Text>
          {notes.map((note) => <Text key={note} style={styles.muted}>• {note}</Text>)}
          {!limits.allowed && <Text style={styles.muted}>{tr({ fi: `Kohde ei läpäise omaa rajaa: edge ${limits.edgeOk ? "OK" : "liian pieni"}, confidence ${limits.confidenceOk ? "OK" : "liian pieni"}.`, en: `The pick does not pass your limits: edge ${limits.edgeOk ? "OK" : "too low"}, confidence ${limits.confidenceOk ? "OK" : "too low"}.`, es: `El pronóstico no supera tus límites: ventaja ${limits.edgeOk ? "OK" : "demasiado baja"}, confianza ${limits.confidenceOk ? "OK" : "demasiado baja"}.` })}</Text>}
          <Field label={tr({ fi: "Paperipanos (€)", en: "Paper stake (€)", es: "Importe simulado (€)" })} value={stakes[id] || initialStake(pick, maximumStake).toFixed(2)} onChangeText={(value) => setStakes((current) => ({ ...current, [id]: value }))} keyboardType="decimal-pad" />
          <ActionButton label={savingId === id ? tr({ fi: "Tallennetaan…", en: "Saving…", es: "Guardando…" }) : tr({ fi: "Lisää paperiseurantaan", en: "Add to paper tracking", es: "Añadir al seguimiento simulado" })} onPress={() => savePick(pick, index)} disabled={savingId !== null || !canSave} />
          {decision === "SKIP" && <Text style={styles.muted}>{tr({ fi: "SKIP tarkoittaa, että hinta tai aineiston laatu ei täytä Scorecasterin rajaa.", en: "SKIP means the price or data quality does not meet the Scorecaster gate.", es: "SKIP significa que la cuota o la calidad de los datos no supera el filtro de Scorecaster." })}</Text>}
        </Card>;
      })}
    </ScrollView>
  );
