...
alter table public.notification_devices add constraint notification_device_platform_allowed check (platform in ('ios', 'android')) not valid;
alter table public.notification_devices drop constraint if exists notification_device_token_format;
alter table public.notification_devices add constraint notification_device_token_format check (expo_push_token ~ '^(ExponentPushToken|ExpoPushToken)\[[A-Za-z0-9_-]{20,200}\]$') not valid;
alter table public.notification_devices drop constraint if exists notification_device_hash_format;
alter table public.notification_devices add constraint notification_device_hash_format check (token_hash ~ '^[a-f0-9]{64}$') not valid;
alter table public.notification_devices drop constraint if exists notification_device_metadata_lengths;
alter table public.notification_devices add constraint notification_device_metadata_lengths check ((app_version is null or char_length(app_version) <= 40) and (build_version is null or char_length(build_version) <= 40)) not valid;

create unique index if not exists idx_notification_devices_token_hash on public.notification_devices(token_hash);
create index if not exists idx_notification_devices_user_enabled on public.notification_devices(user_id, enabled, last_seen_at desc);
...
begin
  if new.push_enabled and not exists (select 1 from public.notification_devices where user_id = new.user_id and enabled = true) then
...
begin
  ...
  if v_user_id is not null and not exists (select 1 from public.notification_devices where user_id = v_user_id and enabled = true) then
    update public.notification_preferences set push_enabled = false where user_id = v_user_id;
  end if;
...
create trigger notification_preferences_set_updated_at before update on public.notification_preferences for each row execute function public.set_notification_updated_at();
drop trigger if exists notification_preferences_enforce_push_state on public.notification_preferences;
create trigger notification_preferences_enforce_push_state before insert or update of push_enabled on public.notification_preferences for each row execute function public.enforce_notification_push_state();
drop trigger if exists notification_devices_set_updated_at on public.notification_devices;
create trigger notification_devices_set_updated_at before update on public.notification_devices for each row execute function public.set_notification_updated_at();
drop trigger if exists notification_devices_sync_push_after_delete on public.notification_devices;
create trigger notification_devices_sync_push_after_delete after delete on public.notification_devices for each row execute function public.sync_notification_push_state_after_device_change();
drop trigger if exists notification_devices_sync_push_after_enabled_update on public.notification_devices;
create trigger notification_devices_sync_push_after_enabled_update after update of enabled on public.notification_devices for each row execute function public.sync_notification_push_state_after_device_change();

drop function if exists public.claim_notification_device(text, text, text, text, text);
create or replace function public.claim_notification_device(p_expo_push_token text, p_platform text, p_app_version text default null, p_build_version text default null)
...
begin
  ...
  if p_expo_push_token !~ '^(ExponentPushToken|ExpoPushToken)\[[A-Za-z0-9_-]{20,200}\]$' then raise exception 'Invalid push token'; end if;
  v_token_hash := encode(digest(p_expo_push_token, 'sha256'), 'hex');
  delete from public.notification_devices where token_hash = v_token_hash and user_id <> v_user_id;
  insert into public.notification_devices (user_id, expo_push_token, token_hash, platform, app_version, build_version, enabled, last_seen_at)
  values (v_user_id, p_expo_push_token, v_token_hash, p_platform, nullif(left(coalesce(p_app_version, ''), 40), ''), nullif(left(coalesce(p_build_version, ''), 40), ''), true, now())
  on conflict (token_hash) do update set user_id = excluded.user_id, expo_push_token = excluded.expo_push_token, platform = excluded.platform, app_version = excluded.app_version, build_version = excluded.build_version, enabled = true, last_seen_at = now()
