-- Fix PL/pgSQL output-variable ambiguity in apply_shadow_candidate_settlements_v1.
...
begin
  ...
  with payload as (
    ...
    where payload.id is not null
      and payload.settlement_status in ('open', 'settled', 'void', 'excluded')
      and (payload.result is null or payload.result in ('win', 'loss', 'push', 'void'))
      and (payload.outcome_value is null or payload.outcome_value in (0, 1))
  ), updated as (
    ...
    returning audit.id as updated_id,
              audit.user_id as updated_user_id,
              audit.settlement_status as updated_settlement_status,
              audit.outcome_value as updated_outcome_value
  )
  select updated.updated_id,
         updated.updated_user_id,
         updated.updated_settlement_status,
         updated.updated_outcome_value
  from updated;
end;
$$;
