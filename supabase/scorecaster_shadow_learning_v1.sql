...
-- Stores immutable paper-decision snapshots and settlement outcomes for governed shadow learning.
-- This migration intentionally contains no model promotion or real-money execution capability.
...
alter table public.shadow_learning_samples
  ...
  check (
    learning_mode = 'shadow-only' and shadow_only = true and
    production_probability_changed = false and real_money_execution = false
  ) not valid;
...
begin
  ...
  v_unified := coalesce(v_raw->'unifiedData', '{}'::jsonb);
  v_factors := case when jsonb_typeof(v_unified->'factors') = 'array' then v_unified->'factors' else '[]'::jsonb end;
  v_audit := coalesce(v_raw->'autonomousAudit', '{}'::jsonb);

  select coalesce(jsonb_agg(item.value order by item.position), '[]'::jsonb)
    into v_used
  from jsonb_array_elements(v_factors) with ordinality as item(value, position)
  where item.value->>'usedByAi' = 'true';

  select coalesce(jsonb_agg(item.value order by item.position), '[]'::jsonb)
    into v_unused
  from jsonb_array_elements(v_factors) with ordinality as item(value, position)
  where coalesce(item.value->>'usedByAi', 'false') <> 'true';

  select coalesce(jsonb_agg(item.value order by item.position), '[]'::jsonb)
    into v_context
  from jsonb_array_elements(v_factors) with ordinality as item(value, position)
  where coalesce(item.value->>'key', '') <> 'odds-consensus';

  select item.value into v_odds_factor
  from jsonb_array_elements(v_factors) as item(value)
  where item.value->>'key' = 'odds-consensus'
  limit 1;

  v_original_probability := coalesce(
    public.shadow_json_numeric(v_raw->'modelProbability'),
    public.shadow_json_numeric(v_raw->'originalProbability')
  );
  ...
  insert into public.shadow_learning_samples (
    ...
    agent_version, model_version, original_probability, market_probability,
    ...
    provider_quality, provider_conflicts, decision_snapshot,
    settlement_status, result, closing_odds, clv, profit, settled_at,
    learning_mode, shadow_only, production_probability_changed, real_money_execution,
    created_at
  ) values (
    ...
    v_original_probability, v_market_probability, v_bet.edge, v_bet.ev,
    v_bet.odds, greatest(0, coalesce(v_bet.stake, 0)),
    coalesce(nullif(v_raw->>'decision', ''), 'PLAY'),
    coalesce(nullif(v_raw->>'decision', ''), 'PLAY'),
    coalesce(v_audit->'reasons', '[]'::jsonb), v_used, v_unused, v_context,
    jsonb_build_object(
      'coverage', coalesce(v_unified->'coverage', '{}'::jsonb),
      'oddsFactor', coalesce(v_odds_factor, '{}'::jsonb),
      'providerSource', v_raw->'providerSource',
      'fixtureSource', v_raw->'fixtureSource'
    ),
    jsonb_build_object(
      'providerDisagreement', v_audit->'data'->'providerDisagreement',
      'warnings', coalesce(v_audit->'warnings', '[]'::jsonb),
      'safetyRecommendation', coalesce(v_unified->'safetyRecommendation', '{}'::jsonb)
    ),
    v_raw,
...
-- Backfill existing governed paper rows. Invalid or incomplete rows remain excluded by the function.
do $$
declare
  v_id uuid;
begin
...
-- Hard boundary: no bookmaker credentials, payment data, deposits, withdrawals or real-money execution exist here.
