-- Scorecaster Autonomous Intelligence V12.1
-- Persistent user-specific control plane for the existing Autonomous Scorecaster V12.
-- Run before scorecaster_settlement_monitor.sql and scorecaster_autonomous_agent.sql.
...
alter table public.autonomous_agent_settings add constraint autonomous_agent_v12_profile_allowed check (autonomy_profile in ('conservative', 'balanced', 'research')) not valid;
alter table public.autonomous_agent_settings drop constraint if exists autonomous_agent_v12_loss_limit;
alter table public.autonomous_agent_settings add constraint autonomous_agent_v12_loss_limit check (max_consecutive_losses between 3 and 20) not valid;
alter table public.autonomous_agent_settings drop constraint if exists autonomous_agent_v12_drawdown_limit;
alter table public.autonomous_agent_settings add constraint autonomous_agent_v12_drawdown_limit check (max_drawdown_percent between 3 and 30) not valid;
alter table public.autonomous_agent_settings drop constraint if exists autonomous_agent_v12_provider_gate;
alter table public.autonomous_agent_settings add constraint autonomous_agent_v12_provider_gate check (minimum_provider_health between 30 and 90) not valid;

alter table public.autonomous_agent_state add column if not exists operating_mode text not null default 'BOOTSTRAP';
...
alter table public.autonomous_agent_state add constraint autonomous_agent_v12_mode_allowed check (operating_mode in ('BOOTSTRAP', 'ACTIVE', 'GUARDED', 'DEGRADED', 'RECOVERY', 'FROZEN')) not valid;
alter table public.autonomous_agent_state drop constraint if exists autonomous_agent_v12_health_range;
alter table public.autonomous_agent_state add constraint autonomous_agent_v12_health_range check (health_score between 0 and 100) not valid;
alter table public.autonomous_agent_state drop constraint if exists autonomous_agent_v12_interval_range;
alter table public.autonomous_agent_state add constraint autonomous_agent_v12_interval_range check (next_interval_minutes between 30 and 1440) not valid;
...
alter table public.autonomous_agent_models add constraint autonomous_agent_model_status_allowed check (status in ('champion', 'challenger', 'retired', 'rejected')) not valid;
...
alter table public.autonomous_agent_models add constraint autonomous_agent_model_published_probability_blocked check (probability_applied_to_published_model = false and paper_risk_policy_only = true) not valid;
alter table public.autonomous_agent_learning_snapshots drop constraint if exists autonomous_agent_learning_health_range;
alter table public.autonomous_agent_learning_snapshots add constraint autonomous_agent_learning_health_range check (health_score between 0 and 100 and sample_size >= 0) not valid;
alter table public.autonomous_agent_incidents drop constraint if exists autonomous_agent_incident_severity_allowed;
alter table public.autonomous_agent_incidents add constraint autonomous_agent_incident_severity_allowed check (severity in ('high', 'medium', 'info')) not valid;

create index if not exists idx_autonomous_models_user_status on public.autonomous_agent_models(user_id, status, updated_at desc);
create index if not exists idx_autonomous_learning_user_captured on public.autonomous_agent_learning_snapshots(user_id, captured_at desc);
create index if not exists idx_autonomous_incidents_user_active on public.autonomous_agent_incidents(user_id, active, severity, last_seen_at desc);
...
create trigger autonomous_agent_models_set_updated_at before update on public.autonomous_agent_models for each row execute function public.set_autonomous_v12_updated_at();
drop trigger if exists autonomous_agent_incidents_set_updated_at on public.autonomous_agent_incidents;
create trigger autonomous_agent_incidents_set_updated_at before update on public.autonomous_agent_incidents for each row execute function public.set_autonomous_v12_updated_at();
...
create or replace function public.complete_autonomous_agent_user_v12(
  ...
  p_operating_mode text default 'BOOTSTRAP',
...
begin
  if p_status not in ('success', 'error', 'deferred') then raise exception 'Unsupported autonomous agent status'; end if;
  if p_operating_mode not in ('BOOTSTRAP', 'ACTIVE', 'GUARDED', 'DEGRADED', 'RECOVERY', 'FROZEN') then raise exception 'Unsupported autonomous operating mode'; end if;
...
create policy "Users read own autonomous models" on public.autonomous_agent_models for select to authenticated using (auth.uid() = user_id);
drop policy if exists "Users read own autonomous learning" on public.autonomous_agent_learning_snapshots;
create policy "Users read own autonomous learning" on public.autonomous_agent_learning_snapshots for select to authenticated using (auth.uid() = user_id);
drop policy if exists "Users read own autonomous incidents" on public.autonomous_agent_incidents;
create policy "Users read own autonomous incidents" on public.autonomous_agent_incidents for select to authenticated using (auth.uid() = user_id);
